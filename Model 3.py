...
class UFCPredictor:
    ...
    def fix_data_leakage(self, df):
        ...
        for idx, row in df.iterrows():
            ...
            for fighter, prefix in [(r_fighter, "r"), (b_fighter, "b")]:
                ...
                # Recent finish rate
                # Adaptive finish rate: use 7 fights if available, then 5, then 3, otherwise 0
                if len(stats["recent_finishes"]) >= 7:
                    df.at[idx, f"{prefix}_recent_finish_rate_corrected"] = sum(stats["recent_finishes"][-7:]) / 7
                elif len(stats["recent_finishes"]) >= 5:
                    df.at[idx, f"{prefix}_recent_finish_rate_corrected"] = sum(stats["recent_finishes"][-7:]) / 5
                elif len(stats["recent_finishes"]) >= 3:
                    df.at[idx, f"{prefix}_recent_finish_rate_corrected"] = sum(stats["recent_finishes"][-3:]) / 3

                # Adaptive recent form: use 7 fights if available, then 5, then 3, otherwise 0
                if len(stats["recent_wins"]) >= 7:
                    df.at[idx, f"{prefix}_recent_form_corrected"] = sum(stats["recent_wins"][-7:]) / 7
                elif len(stats["recent_wins"]) >= 5:
                ...
                # Adaptive window: use 7 fights if available, then 5, then 3, otherwise 0
                if len(stats["recent_wins"]) >= 7:
                    df.at[idx, f"{prefix}_last_5_wins_corrected"] = sum(stats["recent_wins"][-7:])
                elif len(stats["recent_wins"]) >= 5:
                    df.at[idx, f"{prefix}_last_5_wins_corrected"] = sum(stats["recent_wins"][-5:])
                elif len(stats["recent_wins"]) >= 3:
                    df.at[idx, f"{prefix}_last_5_wins_corrected"] = sum(stats["recent_wins"][-3:])
                else:
                    df.at[idx, f"{prefix}_last_5_wins_corrected"] = 0
            ...
            if pd.notna(row["winner"]):
                ...
                for fighter in [r_fighter, b_fighter]:
                    if len(fighter_stats[fighter]["recent_wins"]) > 7:
                        fighter_stats[fighter]["recent_wins"] = fighter_stats[fighter][
                            "recent_wins"
                        ][-7:]
                    if len(fighter_stats[fighter]["recent_finishes"]) > 7:
    ...
    def prepare_upcoming_fight(self, fight, feature_columns):
        ...
        fight_features = {
            ...
            **{
            ...
            # RECENT PERFORMANCE STATS (needed for enhanced method calculation)
            "r_recent_form_corrected": r_stats["recent_form"],
            "b_recent_form_corrected": b_stats["recent_form"],
            "r_recent_finish_rate_corrected": r_stats["recent_finish_rate"],
            "b_recent_finish_rate_corrected": b_stats["recent_finish_rate"],
            "r_win_streak_corrected": r_stats["win_streak"],
            "b_win_streak_corrected": b_stats["win_streak"],
            "r_loss_streak_corrected": r_stats["loss_streak"],
            "b_loss_streak_corrected": b_stats["loss_streak"],
        }
        ...
        # Add total fights to fight_features
        fight_features["r_total_fights"] = r_total_fights
        fight_features["b_total_fights"] = b_total_fights
    ...
    def calculate_comprehensive_method_adjustments(self, fight_data, winner_prefix, loser_prefix):
        """Enhanced method prediction with recent performance emphasis and dynamic weighting"""
        ...
        w_td_def = fight_data[f"{winner_prefix}_pro_td_def_corrected"].values[0]
        ...
        l_slpm = fight_data[f"{loser_prefix}_pro_SLpM_corrected"].values[0]
        l_sig_acc = fight_data[f"{loser_prefix}_pro_sig_str_acc_corrected"].values[0]
        l_td_avg = fight_data[f"{loser_prefix}_pro_td_avg_corrected"].values[0]
        l_sub_avg = fight_data[f"{loser_prefix}_pro_sub_avg_corrected"].values[0]
        l_ko_rate = fight_data[f"{loser_prefix}_ko_rate_corrected"].values[0]
        l_sub_rate = fight_data[f"{loser_prefix}_sub_rate_corrected"].values[0]
        ...
        l_head_pct = fight_data[f"{loser_prefix}_head_pct_corrected"].values[0]
        l_distance_pct = fight_data[f"{loser_prefix}_distance_pct_corrected"].values[0]
        l_clinch_pct = fight_data[f"{loser_prefix}_clinch_pct_corrected"].values[0]
        l_ground_pct = fight_data[f"{loser_prefix}_ground_pct_corrected"].values[0]
        
        # RECENT PERFORMANCE METRICS (critical for method prediction)
        w_recent_form = fight_data[f"{winner_prefix}_recent_form_corrected"].values[0]
        l_recent_form = fight_data[f"{loser_prefix}_recent_form_corrected"].values[0]
        w_recent_finish_rate = fight_data[f"{winner_prefix}_recent_finish_rate_corrected"].values[0]
        l_recent_finish_rate = fight_data[f"{loser_prefix}_recent_finish_rate_corrected"].values[0]
        w_win_streak = fight_data[f"{winner_prefix}_win_streak_corrected"].values[0]
        w_loss_streak = fight_data[f"{winner_prefix}_loss_streak_corrected"].values[0]
        l_win_streak = fight_data[f"{loser_prefix}_win_streak_corrected"].values[0]
        l_loss_streak = fight_data[f"{loser_prefix}_loss_streak_corrected"].values[0]
        
        total_rounds = fight_data["total_rounds"].values[0]
        w_total_fights = fight_data["r_total_fights"].values[0] if winner_prefix == "r" else fight_data["b_total_fights"].values[0]
        l_total_fights = fight_data["b_total_fights"].values[0] if winner_prefix == "r" else fight_data["r_total_fights"].values[0]
        
        # ============================================================================
        # KO/TKO PROBABILITY - Enhanced with recent performance
        # ============================================================================
        ko_base = w_ko_rate
        
        # Recent finish momentum (NEW)
        recent_finish_boost = 1.0
        if w_recent_finish_rate > 0.6:  # Hot streak of finishes
            recent_finish_boost = 1.0 + (w_recent_finish_rate - 0.6) * 1.2
        recent_finish_boost = min(recent_finish_boost, 1.8)
        
        # Recent form amplifier for KO artists (NEW)
        form_amplifier = 1.0 + (w_recent_form - 0.5) * 0.8
        form_amplifier = max(0.7, min(form_amplifier, 1.6))
        
        # Striking volume with accuracy multiplier
        ...
        striking_efficiency = (w_slpm * w_sig_acc) / 3.0
        efficiency_factor = 1.0 + min(striking_efficiency, 1.0)
        
        # Head hunting factor
        head_hunting_factor = 1 + (w_head_pct - 0.5) * 0.7
        head_hunting_factor = max(0.7, min(head_hunting_factor, 1.6))
        
        # Distance striking preference
        distance_factor = 1 + (w_distance_pct - 0.6) * 0.6
        distance_factor = max(0.8, min(distance_factor, 1.5))
        
        # Knockdown threat (major KO predictor)
        kd_threat_factor = 1 + min(w_kd_rate / 0.4, 1.5)
        
        # Opponent vulnerability analysis (enhanced)
        defensive_weakness = (1 - l_str_def) * 1.2
        absorption_vulnerability = (l_sapm / 6.0) * 1.0
        durability_weakness = (1 - l_durability) * 1.3
        
        opp_vulnerability = (
            defensive_weakness * 0.35 +
            absorption_vulnerability * 0.30 +
            durability_weakness * 0.35
        )
        opp_vulnerability = min(opp_vulnerability, 1.2)
        
        # Power differential (striking output vs opponent defense)
        power_differential = (w_slpm * w_sig_acc) - (l_sapm * (1 - l_str_def))
        power_differential_factor = 1 + max(0, power_differential / 4.5)
        power_differential_factor = min(power_differential_factor, 2.0)
        
        # Opponent recent decline factor (NEW) - if loser on losing streak
        opponent_decline = 1.0
        if l_loss_streak >= 2:
            opponent_decline = 1.0 + (l_loss_streak * 0.15)
        opponent_decline = min(opponent_decline, 1.5)
        
        # Pace pressure factor (NEW) - high output vs weak defense
        pace_pressure = (w_slpm - l_slpm) / 3.0
        if pace_pressure > 1.0 and l_str_def < 0.55:
            pace_pressure_factor = 1.0 + min(pace_pressure * 0.2, 0.4)
        else:
            pace_pressure_factor = 1.0
        
        # Combine all KO factors with dynamic weighting
        ko_prob = ko_base * (
            recent_finish_boost * 0.14 +        # Recent finishes
            form_amplifier * 0.12 +             # Recent form
            striking_volume_factor * 0.11 +     # Volume
            accuracy_factor * 0.11 +            # Accuracy
            efficiency_factor * 0.10 +          # Efficiency
            head_hunting_factor * 0.09 +        # Head strikes
            distance_factor * 0.06 +            # Distance work
            kd_threat_factor * 0.12 +           # Knockdowns
            opp_vulnerability * 0.10 +          # Opp weakness
            power_differential_factor * 0.06 +  # Power diff
            opponent_decline * 0.05 +           # Opp decline
            pace_pressure_factor * 0.04         # Pace pressure
        )
        
        # Title fight adjustment
        if total_rounds == 5:
            ko_prob *= 1.18
        
        ko_prob = min(ko_prob, 0.88)
        
        # ============================================================================
        # SUBMISSION PROBABILITY - Enhanced with grappling dynamics
        # ============================================================================
        sub_base = w_sub_rate
        
        # Recent submission momentum (NEW)
        recent_sub_boost = 1.0
        if w_recent_finish_rate > 0.5 and w_sub_rate > 0.3:
            recent_sub_boost = 1.0 + (w_recent_finish_rate * w_sub_rate)
        recent_sub_boost = min(recent_sub_boost, 1.7)
        
        # Grappling control (takedowns * accuracy)
        grappling_control = w_td_avg * w_td_acc
        control_factor = 1 + min(grappling_control / 1.8, 1.2)
        
        # Submission attempt rate
        sub_attempt_factor = 1 + (w_sub_avg / 1.8)
        sub_attempt_factor = min(sub_attempt_factor, 2.2)
        
        # Ground preference
        ground_preference_factor = 1 + (w_ground_pct - 0.2) * 1.0
        ground_preference_factor = max(0.6, min(ground_preference_factor, 1.8))
        
        # Opponent grappling vulnerability (enhanced)
        td_defense_weakness = (1 - l_td_def) * 1.3
        durability_factor = (1 - l_durability) * 0.8
        ground_exposure = l_ground_pct * 1.2
        
        opp_grappling_weakness = (
            td_defense_weakness * 0.45 +
            durability_factor * 0.25 +
            ground_exposure * 0.30
        )
        opp_grappling_weakness = min(opp_grappling_weakness, 1.3)
        
        # Takedown differential
        td_differential = (w_td_avg * w_td_acc) - (l_td_def * 2.2)
        td_factor = 1 + max(0, td_differential / 2.5)
        td_factor = min(td_factor, 2.0)
        
        # Grappling specialist multiplier (NEW)
        if w_sub_rate > 0.35 and w_td_avg > 2.0:
            specialist_bonus = 1.25
        elif w_sub_rate > 0.25 and w_td_avg > 1.5:
            specialist_bonus = 1.12
        else:
            specialist_bonus = 1.0
        
        # Opponent submission vulnerability (NEW) - if they get submitted often
        if l_sub_rate < 0.15 and l_durability > 0.7:  # Durable, doesn't submit
            submission_resistance = 0.75
        else:
            submission_resistance = 1.0
        
        # Clinch control factor (NEW) - clinch leads to takedowns/subs
        clinch_control = (w_clinch_pct - l_clinch_pct) * 0.8
        clinch_factor = 1.0 + max(0, clinch_control)
        clinch_factor = min(clinch_factor, 1.4)
        
        # Combine all submission factors
        sub_prob = sub_base * (
            recent_sub_boost * 0.14 +           # Recent sub finishes
            control_factor * 0.16 +             # Grappling control
            sub_attempt_factor * 0.14 +         # Sub attempts
            ground_preference_factor * 0.12 +   # Ground time
            opp_grappling_weakness * 0.18 +     # Opp weakness
            td_factor * 0.12 +                  # TD differential
            specialist_bonus * 0.08 +           # Specialist bonus
            clinch_factor * 0.06                # Clinch control
        ...
        sub_prob = min(sub_prob, 0.82)
        
        # ============================================================================
        # DECISION PROBABILITY - Enhanced with fight dynamics
        # ============================================================================
        dec_base = w_dec_rate
        
        # Finish threat (inverse relationship with decision)
        finish_threat = ko_prob + sub_prob
        
        # Defensive composite (both fighters)
        winner_defense = (w_sig_acc * 0.5) + ((1 - w_kd_rate/1.0) * 0.5) if w_kd_rate < 1.0 else w_sig_acc * 0.5
        loser_defense = (l_str_def + l_td_def) / 2.0
        defensive_factor = 1 + (winner_defense * 0.3 + loser_defense * 0.7)
        defensive_factor = min(defensive_factor, 1.8)
        
        # Volume without finish (point fighting)
        volume_no_finish = (w_slpm / 6.0) * (1 - w_ko_rate - w_sub_rate)
        volume_decision_factor = 1 + min(volume_no_finish, 0.8)
        
        # Style variety (balanced attack = more likely decision)
        w_style_balance = 1 - abs(w_distance_pct - 0.5) * 2
        variety_factor = 1 + w_style_balance * 0.4
        
        # Experience factor (NEW) - more experienced = more decisions
        experience_ratio = w_total_fights / max(l_total_fights, 1)
        if w_total_fights > 15 and experience_ratio > 1.3:
            experience_factor = 1.15
        elif w_total_fights > 10:
            experience_factor = 1.08
        else:
            experience_factor = 1.0
        
        # Conservative recent form (NEW) - winning without finishes
        if w_recent_form > 0.6 and w_recent_finish_rate < 0.4:
            conservative_factor = 1.25
        else:
            conservative_factor = 1.0
        
        # Opponent durability (NEW) - durable opponents go to decision
        if l_durability > 0.75 and (l_str_def > 0.55 or l_td_def > 0.65):
            durability_decision_boost = 1.3
        elif l_durability > 0.6:
            durability_decision_boost = 1.15
        else:
            durability_decision_boost = 1.0
        
        # Pace matching (NEW) - similar pace = decision likely
        pace_difference = abs(w_slpm - l_slpm)
        if pace_difference < 1.5:  # Similar pace
            pace_match_factor = 1.2
        else:
            pace_match_factor = 1.0
        
        # Combine decision factors
        dec_prob = dec_base * (
            defensive_factor * 0.18 +
            volume_decision_factor * 0.16 +
            variety_factor * 0.14 +
            experience_factor * 0.12 +
            conservative_factor * 0.14 +
            durability_decision_boost * 0.16 +
            pace_match_factor * 0.10
        )
        
        # Strong inverse relationship with finish threat
        dec_prob *= (1 - finish_threat * 0.6)
        
        # Title fights slightly less likely to end in decision (more urgency)
        if total_rounds == 5:
            dec_prob *= 0.93
        
        # ============================================================================
        # ADVANCED STYLE MATCHUP ADJUSTMENTS
        # ============================================================================
        
        # Striker vs Grappler dynamics
        w_striker_score = (w_distance_pct * 1.2) + (w_head_pct * 0.8) - (w_ground_pct * 1.0)
        l_striker_score = (l_distance_pct * 1.2) + (l_head_pct * 0.8) - (l_ground_pct * 1.0)
        striker_advantage = w_striker_score - l_striker_score
        
        if striker_advantage > 0.4:  # Winner is much more striker
            ko_prob *= 1.22
            sub_prob *= 0.80
            dec_prob *= 1.05
        elif striker_advantage < -0.4:  # Winner is much more grappler
            sub_prob *= 1.22
            ko_prob *= 0.80
            dec_prob *= 1.05
        
        # Pressure fighter dynamics (NEW)
        pressure_differential = (w_slpm + w_td_avg) - (l_slpm + l_td_avg)
        if pressure_differential > 3 and l_str_def < 0.55:
            ko_prob *= 1.15
            dec_prob *= 0.92
        elif pressure_differential > 3:
            dec_prob *= 1.10
        
        # Counter-striker dynamics (NEW)
        if w_sig_acc > 0.50 and w_slpm < 4.0 and l_slpm > 5.0:  # Counter-striker
            ko_prob *= 1.12
            dec_prob *= 1.08
        
        # Scramble specialist (NEW)
        if w_sub_avg > 1.5 and w_td_def > 0.65:  # Good at scrambles
            sub_prob *= 1.15
        
        # Glass cannon check (NEW) - high output but poor defense
        if w_slpm > 6.0 and w_sig_acc > 0.48 and l_ko_rate > 0.3:
            ko_prob *= 1.18
            dec_prob *= 0.90
        
        # ============================================================================
        # MOMENTUM AND RECENT PERFORMANCE MULTIPLIERS
        # ============================================================================
        
        # Hot streak amplification (NEW)
        if w_win_streak >= 3:
            streak_multiplier = 1.0 + (w_win_streak * 0.06)
            streak_multiplier = min(streak_multiplier, 1.25)
            
            # Apply to primary method
            if w_ko_rate > max(w_sub_rate, w_dec_rate):
                ko_prob *= streak_multiplier
            elif w_sub_rate > max(w_ko_rate, w_dec_rate):
                sub_prob *= streak_multiplier
            else:
                dec_prob *= streak_multiplier
        
        # Recent finish pattern (NEW)
        if w_recent_finish_rate > 0.65:
            finish_hot_multiplier = 1.15
            ko_prob *= finish_hot_multiplier
            sub_prob *= finish_hot_multiplier
            dec_prob *= 0.85
        ...
        ko_prob = max(0.05, min(ko_prob, 0.88))
        sub_prob = max(0.05, min(sub_prob, 0.82))
        dec_prob = max(0.08, min(dec_prob, 0.85))
