...
class AdvancedUFCPredictor:
    ...
    def fix_data_leakage(self, df):
        ...
        for idx, row in df.iterrows():
            ...
            for fighter, prefix in [(r_fighter, "r"), (b_fighter, "b")]:
                ...
                df.at[idx, f"{prefix}_total_fights_corrected"] = stats["wins"] + stats["losses"] + stats["draws"]
                ...
                fight_time = max(stats["fight_time_minutes"], 0.001)
                df.at[idx, f"{prefix}_pro_SLpM_corrected"] = (
                    stats["sig_str_total"] / fight_time
                )
                df.at[idx, f"{prefix}_pro_SApM_corrected"] = (
                    stats["sig_str_absorbed_total"] / fight_time
                )
                df.at[idx, f"{prefix}_pro_total_str_pM_corrected"] = (
                    stats["total_str_landed"] / fight_time
                )
                df.at[idx, f"{prefix}_pro_total_str_absorbed_pM_corrected"] = (
                    stats["total_str_absorbed"] / fight_time
                )
                df.at[idx, f"{prefix}_pro_td_avg_corrected"] = (
                    stats["td_total"] / fight_time
                ) * 15
                df.at[idx, f"{prefix}_pro_sub_avg_corrected"] = (
                    stats["sub_att_total"] / fight_time
                ) * 15
                df.at[idx, f"{prefix}_pro_kd_pM_corrected"] = (
                    stats["kd_total"] / fight_time
                )
                df.at[idx, f"{prefix}_kd_pM_corrected"] = (
                    stats["kd_total"] / fight_time
                )
                
                # Ape index (reach/height ratio) - use original values as they don't change over time
                if f"{prefix}_ape_index" in df.columns:
                    df.at[idx, f"{prefix}_ape_index_corrected"] = df.at[idx, f"{prefix}_ape_index"]
                
                # Strike target counts (head, body, leg) - use original values as they don't change over time
                if f"{prefix}_head" in df.columns:
                    df.at[idx, f"{prefix}_head_corrected"] = df.at[idx, f"{prefix}_head"]
                if f"{prefix}_body" in df.columns:
                    df.at[idx, f"{prefix}_body_corrected"] = df.at[idx, f"{prefix}_body"]
                if f"{prefix}_leg" in df.columns:
                    df.at[idx, f"{prefix}_leg_corrected"] = df.at[idx, f"{prefix}_leg"]
    ...
    def create_neutral_features(self, df):
        ...
        # Use corrected total_fights to prevent data leakage
        r_total_fights_col = self.get_col_name(df, "total_fights", "r")
        b_total_fights_col = self.get_col_name(df, "total_fights", "b")
        
        if all(col for col in [r_total_fights_col, b_total_fights_col]):
            df["experience_mismatch"] = np.abs(df[r_total_fights_col] - df[b_total_fights_col])
        else:
            df["experience_mismatch"] = 0.0
    ...
    def get_col_name(self, df, base_name, prefix="r"):
        """Helper function to get column name (corrected if available, raw as fallback)"""
        corrected_name = f"{prefix}_{base_name}_corrected"
        raw_name = f"{prefix}_{base_name}"
    ...
    def prepare_features(self, df):
        ...
        feature_columns = [
            ...
            "pace_differential",
            # Additional time and experience features
            "r_avg_fight_time",
            "b_avg_fight_time", 
            "avg_fight_time_diff",
            "r_adversity_experience",
            "b_adversity_experience",
        ]
        ...
        if all(col for col in [r_total_fights_col, b_total_fights_col]):
            df["experience_gap"] = df[r_total_fights_col] - df[b_total_fights_col]
            df["experience_ratio"] = df[r_total_fights_col] / (df[b_total_fights_col] + 1)
        else:
            df["experience_gap"] = 0.0
            df["experience_ratio"] = 1.0

        df["r_avg_fight_time"] = df["r_fight_time_minutes_corrected"] / (
            df[r_total_fights_col] + 1
        )
        df["b_avg_fight_time"] = df["b_fight_time_minutes_corrected"] / (
            df[b_total_fights_col] + 1
        )
        ...
        df["r_elite_fight_ratio"] = np.where(
            df[r_total_fights_col] > 0, df["r_wins_corrected"] / df[r_total_fights_col], 0
        )
        df["b_elite_fight_ratio"] = np.where(
            df[b_total_fights_col] > 0, df["b_wins_corrected"] / df[b_total_fights_col], 0
        )

        df["quality_experience_gap"] = (
            df[r_total_fights_col] * df["r_elite_fight_ratio"]
            - df[b_total_fights_col] * df["b_elite_fight_ratio"]
        )

        df["r_championship_experience"] = np.where(
            df[r_total_fights_col] > 0,
            (df["r_fight_time_minutes_corrected"] / df[r_total_fights_col]) > 12,
            0,
        ).astype(int)
        df["b_championship_experience"] = np.where(
            df[b_total_fights_col] > 0,
            (df["b_fight_time_minutes_corrected"] / df[b_total_fights_col]) > 12,
            0,
        ).astype(int)
        ...
        df["novice_vulnerability"] = np.where(
            (df[r_total_fights_col] < 10) | (df[b_total_fights_col] < 10),
        ...
        df["momentum_score_advantage"] = df["r_momentum_score"] - df["b_momentum_score"]
        ...
        df["upset_potential"] = (
            ...
            + (df[b_total_fights_col] - df[r_total_fights_col]) * 0.2
        )
        ...
        # ADDITIONAL SUPPORTING FEATURES FOR NEW FEATURES
        # ============================================================================
        
        # Create fights_in_last_year if not exists - more realistic approximation
        if "r_fights_in_last_year" not in df.columns:
            # More realistic: recent activity based on total fights and age
            df["r_fights_in_last_year"] = np.minimum(
                df[r_total_fights_col] * 0.15,  # Max 15% of career fights in last year
                np.maximum(1, df[r_total_fights_col] / 5)  # At least 1 fight if experienced
            )
            df["b_fights_in_last_year"] = np.minimum(
                df[b_total_fights_col] * 0.15,
                np.maximum(1, df[b_total_fights_col] / 5)
            )
        
        # Create decision_wins if not exists - use actual decision rate if available
        if "r_decision_wins" not in df.columns:
            # Use decision rate from method data if available, otherwise estimate based on KO rate
            if "r_dec_rate_corrected" in df.columns:
                df["r_decision_wins"] = df["r_wins_corrected"] * df["r_dec_rate_corrected"]
                df["b_decision_wins"] = df["b_wins_corrected"] * df["b_dec_rate_corrected"]
            else:
                # Estimate based on KO rate (higher KO rate = fewer decisions)
                df["r_decision_wins"] = df["r_wins_corrected"] * (1 - df["r_ko_rate_corrected"] * 0.8)
                df["b_decision_wins"] = df["b_wins_corrected"] * (1 - df["b_ko_rate_corrected"] * 0.8)
        
        # Create kd_pM (knockdowns per minute) if not exists
        if "r_kd_pM" not in df.columns:
            if "r_pro_kd_pM_corrected" in df.columns:
                df["r_kd_pM"] = df["r_pro_kd_pM_corrected"]
                df["b_kd_pM"] = df["b_pro_kd_pM_corrected"]
            else:
                # Estimate based on KO rate and striking volume
                df["r_kd_pM"] = df["r_pro_SLpM_corrected"] * df["r_ko_rate_corrected"] * 0.1
                df["b_kd_pM"] = df["b_pro_SLpM_corrected"] * df["b_ko_rate_corrected"] * 0.1
        
        # Create missing columns that are needed for new features
        # Note: These are raw totals (including current fight), corrected versions are created in fix_data_leakage()
        if "r_total_fights" not in df.columns:
            df["r_total_fights"] = df["r_wins"] + df["r_losses"] + df["r_draws"]
            df["b_total_fights"] = df["b_wins"] + df["b_losses"] + df["b_draws"]
        
        if "r_win_streak" not in df.columns:
            # Estimate win streak based on recent form (simplified)
            df["r_win_streak"] = np.maximum(0, df["r_wins"] - df["r_losses"]) / 2
            df["b_win_streak"] = np.maximum(0, df["b_wins"] - df["b_losses"]) / 2
        
        # Create head/body/leg strike percentages if not exists - use actual data if available
        if "r_head_percentage" not in df.columns:
            if "r_head_pct_corrected" in df.columns:
                df["r_head_percentage"] = df["r_head_pct_corrected"] / 100
                df["b_head_percentage"] = df["b_head_pct_corrected"] / 100
                df["r_body_percentage"] = df["r_body_pct_corrected"] / 100
                df["b_body_percentage"] = df["b_body_pct_corrected"] / 100
                df["r_leg_percentage"] = df["r_leg_pct_corrected"] / 100
                df["b_leg_percentage"] = df["b_leg_pct_corrected"] / 100
            else:
                # More realistic defaults based on fighter style
                # Strikers tend to target head more, grapplers target body more
                df["r_head_percentage"] = np.where(
                    df["r_pro_SLpM"] > df["r_pro_td_avg"], 0.65, 0.55
                )
                df["b_head_percentage"] = np.where(
                    df["b_pro_SLpM"] > df["b_pro_td_avg"], 0.65, 0.55
                )
                df["r_body_percentage"] = np.where(
                    df["r_pro_td_avg"] > 2, 0.30, 0.25
                )
                df["b_body_percentage"] = np.where(
                    df["b_pro_td_avg"] > 2, 0.30, 0.25
                )
                df["r_leg_percentage"] = 1 - df["r_head_percentage"] - df["r_body_percentage"]
                df["b_leg_percentage"] = 1 - df["b_head_percentage"] - df["b_body_percentage"]
        
        # FINISH RATE - Recent finishing ability (use available columns)
        if all(col in df.columns for col in ["r_ko_rate_corrected", "r_sub_rate_corrected", "b_ko_rate_corrected", "b_sub_rate_corrected"]):
            ...
            # Estimate finish rate from available data
            df["r_finish_rate"] = df["r_wins"] / (df["r_wins"] + df["r_losses"] + 0.001) * 0.5  # Rough estimate
            df["b_finish_rate"] = df["b_wins"] / (df["b_wins"] + df["b_losses"] + 0.001) * 0.5
            df["finish_rate_diff"] = df["r_finish_rate"] - df["b_finish_rate"]
        ...
        # ============================================================================
        # 26 NEW HIGH-IMPACT FEATURES (24 + 2 STANCE FEATURES)
        # ============================================================================
        
        # Check if corrected columns exist, if not use raw columns as fallback
        has_corrected_cols = any('_corrected' in col for col in df.columns)
        if not has_corrected_cols:
            print("⚠️  WARNING: No corrected columns found. Using raw columns (may have data leakage).")
            print("   Consider running fix_data_leakage() first for proper chronological accuracy.")
        
        
        # POWER VS VOLUME ANALYSIS
        r_kd_col = self.get_col_name(df, "kd_pM", "r")
        r_slpm_col = self.get_col_name(df, "pro_SLpM", "r")
        b_kd_col = self.get_col_name(df, "kd_pM", "b")
        b_slpm_col = self.get_col_name(df, "pro_SLpM", "b")
        
        if all(col for col in [r_kd_col, r_slpm_col, b_kd_col, b_slpm_col]):
            df["r_power_strike_ratio"] = df[r_kd_col] / (df[r_slpm_col] + 0.001)
            df["b_power_strike_ratio"] = df[b_kd_col] / (df[b_slpm_col] + 0.001)
            df["power_strike_ratio_diff"] = df["r_power_strike_ratio"] - df["b_power_strike_ratio"]
        else:
            df["power_strike_ratio_diff"] = 0.0
        
        # STRIKE EFFICIENCY GAP
        r_sig_str_acc_col = self.get_col_name(df, "pro_sig_str_acc", "r")
        r_sapm_col = self.get_col_name(df, "pro_SApM", "r")
        b_sig_str_acc_col = self.get_col_name(df, "pro_sig_str_acc", "b")
        b_sapm_col = self.get_col_name(df, "pro_SApM", "b")
        
        if all(col for col in [r_sig_str_acc_col, r_sapm_col, b_sig_str_acc_col, b_sapm_col]):
            df["r_strike_efficiency"] = df[r_sig_str_acc_col] - df[r_sapm_col]
            df["b_strike_efficiency"] = df[b_sig_str_acc_col] - df[b_sapm_col]
            df["strike_efficiency_gap"] = df["r_strike_efficiency"] - df["b_strike_efficiency"]
        else:
            df["strike_efficiency_gap"] = 0.0
        ...
        if all(col for col in [r_slpm_col, r_sapm_col, b_slpm_col, b_sapm_col]):
            df["r_damage_ratio"] = df[r_slpm_col] / (df[r_sapm_col] + 0.001)
            df["b_damage_ratio"] = df[b_slpm_col] / (df[b_sapm_col] + 0.001)
            df["damage_ratio_diff"] = df["r_damage_ratio"] - df["b_damage_ratio"]
        else:
            df["damage_ratio_diff"] = 0.0
        
        # GROUND CONTROL MASTERY
        r_td_acc_col = self.get_col_name(df,"pro_td_acc", "r")
        r_td_avg_col = self.get_col_name(df,"pro_td_avg", "r")
        b_td_acc_col = self.get_col_name(df,"pro_td_acc", "b")
        b_td_avg_col = self.get_col_name(df,"pro_td_avg", "b")
        
        if all(col for col in [r_td_acc_col, r_td_avg_col, b_td_acc_col, b_td_avg_col]):
            df["r_takedown_efficiency"] = df[r_td_acc_col] * df[r_td_avg_col]
            df["b_takedown_efficiency"] = df[b_td_acc_col] * df[b_td_avg_col]
            df["takedown_efficiency_diff"] = df["r_takedown_efficiency"] - df["b_takedown_efficiency"]
        else:
            df["takedown_efficiency_diff"] = 0.0
        
        # GROUND CONTROL RATIO
        r_td_avg_col = self.get_col_name(df,"pro_td_avg", "r")
        r_sub_avg_col = self.get_col_name(df,"pro_sub_avg", "r")
        b_td_avg_col = self.get_col_name(df,"pro_td_avg", "b")
        b_sub_avg_col = self.get_col_name(df,"pro_sub_avg", "b")
        
        if all(col for col in [r_td_avg_col, r_sub_avg_col, b_td_avg_col, b_sub_avg_col]):
            df["r_ground_control_ratio"] = df[r_td_avg_col] / (df[r_sub_avg_col] + 0.001)
            df["b_ground_control_ratio"] = df[b_td_avg_col] / (df[b_sub_avg_col] + 0.001)
            df["ground_control_ratio_diff"] = df["r_ground_control_ratio"] - df["b_ground_control_ratio"]
        else:
            df["ground_control_ratio_diff"] = 0.0
        
        # GRAPPLING BALANCE
        if all(col for col in [r_td_avg_col, r_sub_avg_col, b_td_avg_col, b_sub_avg_col]):
            df["r_grappling_balance"] = (df[r_td_avg_col] + df[r_sub_avg_col]) / 2
            df["b_grappling_balance"] = (df[b_td_avg_col] + df[b_sub_avg_col]) / 2
            df["grappling_balance_diff"] = df["r_grappling_balance"] - df["b_grappling_balance"]
        else:
            df["grappling_balance_diff"] = 0.0
        
        # DEFENSIVE SOPHISTICATION
        r_str_def_col = self.get_col_name(df,"pro_str_def", "r")
        r_sapm_col = self.get_col_name(df,"pro_SApM", "r")
        b_str_def_col = self.get_col_name(df,"pro_str_def", "b")
        b_sapm_col = self.get_col_name(df,"pro_SApM", "b")
        
        if all(col for col in [r_str_def_col, r_sapm_col, b_str_def_col, b_sapm_col]):
            df["r_defensive_ratio"] = df[r_str_def_col] / (df[r_sapm_col] + 0.001)
            df["b_defensive_ratio"] = df[b_str_def_col] / (df[b_sapm_col] + 0.001)
            df["defensive_ratio_diff"] = df["r_defensive_ratio"] - df["b_defensive_ratio"]
        else:
            df["defensive_ratio_diff"] = 0.0
        
        # TAKEDOWN DEFENSE STRENGTH
        r_td_def_col = self.get_col_name(df,"pro_td_def", "r")
        b_td_def_col = self.get_col_name(df,"pro_td_def", "b")
        
        if all(col for col in [r_td_def_col, r_sapm_col, b_td_def_col, b_sapm_col]):
            df["r_takedown_defense_strength"] = df[r_td_def_col] * (1 - df[r_sapm_col])
            df["b_takedown_defense_strength"] = df[b_td_def_col] * (1 - df[b_sapm_col])
            df["takedown_defense_strength_diff"] = df["r_takedown_defense_strength"] - df["b_takedown_defense_strength"]
        else:
            df["takedown_defense_strength_diff"] = 0.0
        
        # OVERALL DEFENSE SCORE
        if all(col for col in [r_str_def_col, r_td_def_col, b_str_def_col, b_td_def_col]):
            df["r_overall_defense_score"] = (df[r_str_def_col] + df[r_td_def_col]) / 2
            df["b_overall_defense_score"] = (df[b_str_def_col] + df[b_td_def_col]) / 2
            df["overall_defense_score_diff"] = df["r_overall_defense_score"] - df["b_overall_defense_score"]
        else:
            df["overall_defense_score_diff"] = 0.0
        
        # CAREER PROGRESSION
        r_total_fights_col = self.get_col_name(df, "total_fights", "r")
        b_total_fights_col = self.get_col_name(df, "total_fights", "b")
        
        if all(col for col in [r_total_fights_col, "r_age_at_event", b_total_fights_col, "b_age_at_event"]):
            df["r_experience_factor"] = df[r_total_fights_col] / (df["r_age_at_event"] + 0.001)
            df["b_experience_factor"] = df[b_total_fights_col] / (df["b_age_at_event"] + 0.001)
            df["experience_factor_diff"] = df["r_experience_factor"] - df["b_experience_factor"]
        else:
            df["experience_factor_diff"] = 0.0
        
        # WIN RATE CONSISTENCY
        r_wins_col = self.get_col_name(df,"wins", "r")
        r_losses_col = self.get_col_name(df,"losses", "r")
        b_wins_col = self.get_col_name(df,"wins", "b")
        b_losses_col = self.get_col_name(df,"losses", "b")
        
        if all(col for col in [r_wins_col, r_losses_col, b_wins_col, b_losses_col]):
            df["r_win_rate_consistency"] = df[r_wins_col] / (df[r_wins_col] + df[r_losses_col] + 0.001)
            df["b_win_rate_consistency"] = df[b_wins_col] / (df[b_wins_col] + df[b_losses_col] + 0.001)
            df["win_rate_consistency_diff"] = df["r_win_rate_consistency"] - df["b_win_rate_consistency"]
        else:
            df["win_rate_consistency_diff"] = 0.0
        
        # Use the finish rate already calculated above (ko_rate + sub_rate) / 2
        # Don't recalculate with decision_wins to avoid circular logic
        df["finish_rate_diff"] = df["r_finish_rate"] - df["b_finish_rate"]
        
        # STANCE AND PHYSICAL MISMATCH
        # Check if stance data is available
        if "r_stance" in df.columns and "b_stance" in df.columns:
            # Orthodox vs Southpaw (existing)
            df["stance_advantage"] = df["stance_matchup_advantage"]  # Use existing feature
            
            # Orthodox vs Switch - CORRECTED: Switch has advantage
            df["orthodox_vs_switch"] = np.where(
                (df["r_stance"] == "Orthodox") & (df["b_stance"] == "Switch"), -1,  # Blue Switch advantage
                np.where((df["r_stance"] == "Switch") & (df["b_stance"] == "Orthodox"), 1, 0)  # Red Switch advantage
            )
            
            # Southpaw vs Switch - CORRECTED: Switch has advantage
            df["southpaw_vs_switch"] = np.where(
                (df["r_stance"] == "Southpaw") & (df["b_stance"] == "Switch"), -1,  # Blue Switch advantage
                np.where((df["r_stance"] == "Switch") & (df["b_stance"] == "Southpaw"), 1, 0)  # Red Switch advantage
            )
            
            # Orthodox vs Southpaw - ADDED: Southpaw has advantage
            df["orthodox_vs_southpaw"] = np.where(
                (df["r_stance"] == "Orthodox") & (df["b_stance"] == "Southpaw"), -1,  # Blue Southpaw advantage
                np.where((df["r_stance"] == "Southpaw") & (df["b_stance"] == "Orthodox"), 1, 0)  # Red Southpaw advantage
            )
        else:
            # Default to neutral if stance data is missing
            df["stance_advantage"] = 0
            df["orthodox_vs_switch"] = 0
            df["southpaw_vs_switch"] = 0
            df["orthodox_vs_southpaw"] = 0
        
        # REACH UTILIZATION
        r_ape_index_col = self.get_col_name(df,"ape_index", "r")
        b_ape_index_col = self.get_col_name(df,"ape_index", "b")
        
        if all(col for col in [r_ape_index_col, r_slpm_col, b_ape_index_col, b_slpm_col]):
            df["r_reach_utilization"] = df[r_ape_index_col] * df[r_slpm_col]
            df["b_reach_utilization"] = df[b_ape_index_col] * df[b_slpm_col]
            df["reach_utilization_diff"] = df["r_reach_utilization"] - df["b_reach_utilization"]
        else:
            df["reach_utilization_diff"] = 0.0
        
        df["size_advantage_factor"] = (df["height_diff"] + df["reach_diff"]) / 2
        
        # EVENT AND ROUND CONTEXT
        # TITLE FIGHT EXPERIENCE
        if all(col for col in ["is_title_bout", r_wins_col, r_losses_col, b_wins_col, b_losses_col]):
            df["r_title_fight_experience"] = df["is_title_bout"] * (df[r_wins_col] + df[r_losses_col])
            df["b_title_fight_experience"] = df["is_title_bout"] * (df[b_wins_col] + df[b_losses_col])
            df["title_fight_experience_diff"] = df["r_title_fight_experience"] - df["b_title_fight_experience"]
        else:
            df["title_fight_experience_diff"] = 0.0
        
        # ROUND SPECIALIST
        if all(col for col in ["total_rounds", r_slpm_col, b_slpm_col]):
            df["r_round_specialist"] = df["total_rounds"] * (df[r_slpm_col] / 5)
            df["b_round_specialist"] = df["total_rounds"] * (df[b_slpm_col] / 5)
            df["round_specialist_diff"] = df["r_round_specialist"] - df["b_round_specialist"]
        else:
            df["round_specialist_diff"] = 0.0
        
        # MAIN EVENT PRESSURE
        if all(col for col in ["is_title_bout", r_slpm_col, b_slpm_col]):
            df["r_main_event_pressure"] = df["is_title_bout"] * df[r_slpm_col]
            df["b_main_event_pressure"] = df["is_title_bout"] * df[b_slpm_col]
            df["main_event_pressure_diff"] = df["r_main_event_pressure"] - df["b_main_event_pressure"]
        else:
            df["main_event_pressure_diff"] = 0.0
        
        # TARGET DISTRIBUTION INTELLIGENCE
        r_head_col = self.get_col_name(df,"head", "r")
        r_body_col = self.get_col_name(df,"body", "r")
        r_leg_col = self.get_col_name(df,"leg", "r")
        b_head_col = self.get_col_name(df,"head", "b")
        b_body_col = self.get_col_name(df,"body", "b")
        b_leg_col = self.get_col_name(df,"leg", "b")
        
        if all(col for col in [r_head_col, r_body_col, r_leg_col, r_slpm_col, b_head_col, b_body_col, b_leg_col, b_slpm_col]):
            # Convert head strikes to percentage of total strikes
            df["r_head_percentage"] = df[r_head_col] / (df[r_head_col] + df[r_body_col] + df[r_leg_col] + 0.001)
            df["b_head_percentage"] = df[b_head_col] / (df[b_head_col] + df[b_body_col] + df[b_leg_col] + 0.001)
            df["r_head_hunting_ratio"] = df["r_head_percentage"] * df[r_slpm_col]
            df["b_head_hunting_ratio"] = df["b_head_percentage"] * df[b_slpm_col]
            df["head_hunting_ratio_diff"] = df["r_head_hunting_ratio"] - df["b_head_hunting_ratio"]
        else:
            df["head_hunting_ratio_diff"] = 0.0
        
        # BODY WORK FREQUENCY
        if all(col for col in [r_body_col, b_body_col, r_slpm_col, b_slpm_col]):
            df["r_body_percentage"] = df[r_body_col] / (df[r_head_col] + df[r_body_col] + df[r_leg_col] + 0.001)
            df["b_body_percentage"] = df[b_body_col] / (df[b_head_col] + df[b_body_col] + df[b_leg_col] + 0.001)
            df["r_body_work_frequency"] = df["r_body_percentage"] * df[r_slpm_col]
            df["b_body_work_frequency"] = df["b_body_percentage"] * df[b_slpm_col]
            df["body_work_frequency_diff"] = df["r_body_work_frequency"] - df["b_body_work_frequency"]
        else:
            df["body_work_frequency_diff"] = 0.0
        
        # LEG ATTACK FREQUENCY
        if all(col for col in [r_leg_col, b_leg_col, r_slpm_col, b_slpm_col]):
            df["r_leg_percentage"] = df[r_leg_col] / (df[r_head_col] + df[r_body_col] + df[r_leg_col] + 0.001)
            df["b_leg_percentage"] = df[b_leg_col] / (df[b_head_col] + df[b_body_col] + df[b_leg_col] + 0.001)
            df["r_leg_attack_frequency"] = df["r_leg_percentage"] * df[r_slpm_col]
            df["b_leg_attack_frequency"] = df["b_leg_percentage"] * df[b_slpm_col]
            df["leg_attack_frequency_diff"] = df["r_leg_attack_frequency"] - df["b_leg_attack_frequency"]
        else:
            df["leg_attack_frequency_diff"] = 0.0
        
        # RECENT PERFORMANCE TRENDS
        # RECENT ACTIVITY LEVEL
        if all(col for col in ["r_fights_in_last_year", r_total_fights_col, "b_fights_in_last_year", b_total_fights_col]):
            df["r_recent_activity_level"] = df["r_fights_in_last_year"] / (df[r_total_fights_col] + 0.001)
            df["b_recent_activity_level"] = df["b_fights_in_last_year"] / (df[b_total_fights_col] + 0.001)
            df["recent_activity_level_diff"] = df["r_recent_activity_level"] - df["b_recent_activity_level"]
        else:
            df["recent_activity_level_diff"] = 0.0
        
        # FORM CONSISTENCY (using win rate consistency instead to avoid duplication)
        if all(col for col in [r_wins_col, r_losses_col, b_wins_col, b_losses_col]):
            df["r_form_consistency_alt"] = df[r_wins_col] / (df[r_wins_col] + df[r_losses_col] + 0.001)
            df["b_form_consistency_alt"] = df[b_wins_col] / (df[b_wins_col] + df[b_losses_col] + 0.001)
            df["form_consistency_alt_diff"] = df["r_form_consistency_alt"] - df["b_form_consistency_alt"]
        else:
            df["form_consistency_alt_diff"] = 0.0
        
        # MOMENTUM FACTOR
        r_win_streak_col = self.get_col_name(df,"win_streak", "r")
        b_win_streak_col = self.get_col_name(df,"win_streak", "b")
        
        if all(col for col in [r_win_streak_col, r_slpm_col, b_win_streak_col, b_slpm_col]):
            df["r_momentum_factor"] = df[r_win_streak_col] * df[r_slpm_col]
            df["b_momentum_factor"] = df[b_win_streak_col] * df[b_slpm_col]
            df["momentum_factor_diff"] = df["r_momentum_factor"] - df["b_momentum_factor"]
        else:
            df["momentum_factor_diff"] = 0.0
        ...
        feature_columns.extend(
            [
                ...
                "r_elite_fight_ratio",
                "b_elite_fight_ratio",
                "r_championship_experience",
                "b_championship_experience",
                "r_adversity_experience",
                "b_adversity_experience",
                "adversity_exp_diff",
                "r_striker_score",
                ...
                "r_kd_resistance",
                "b_kd_resistance",
                "r_opponent_vulnerability",
                "b_opponent_vulnerability",
                "r_momentum_score",
                "b_momentum_score",
                "momentum_score_advantage",
                "r_power_technique_ratio",
                "b_power_technique_ratio",
                "r_grappling_threat",
                "b_grappling_threat",
                "r_recent_ko_trend",
                "b_recent_ko_trend",
                "r_opponent_ko_susceptibility",
                "b_opponent_ko_susceptibility",
                "r_sub_opportunity_score",
                "b_sub_opportunity_score",
                "r_cardio_factor",
                "b_cardio_factor",
                "r_championship_pressure",
                "b_championship_pressure",
                "r_opponent_quality",
                "b_opponent_quality",
                "r_technical_striker",
                "b_technical_striker",
                ...
                "r_recent_opponent_quality",
                "b_recent_opponent_quality",
                "r_upset_history",
                "b_upset_history",
                "r_power_strike_ratio",
                "b_power_strike_ratio",
                "r_strike_efficiency",
                "b_strike_efficiency",
                "r_damage_ratio",
                "b_damage_ratio",
                "r_takedown_efficiency",
                "b_takedown_efficiency",
                "r_ground_control_ratio",
                "b_ground_control_ratio",
                "r_grappling_balance",
                "b_grappling_balance",
                "r_defensive_ratio",
                "b_defensive_ratio",
                "r_takedown_defense_strength",
                "b_takedown_defense_strength",
                "r_overall_defense_score",
                "b_overall_defense_score",
                "r_experience_factor",
                "b_experience_factor",
                "r_win_rate_consistency",
                "b_win_rate_consistency",
                "r_finish_rate",
                "b_finish_rate",
                "r_reach_utilization",
                "b_reach_utilization",
                "r_title_fight_experience",
                "b_title_fight_experience",
                "r_round_specialist",
                "b_round_specialist",
                "r_main_event_pressure",
                "b_main_event_pressure",
                "r_head_percentage",
                "b_head_percentage",
                "r_body_percentage",
                "b_body_percentage",
                "r_leg_percentage",
                "b_leg_percentage",
                "r_head_hunting_ratio",
                "b_head_hunting_ratio",
                "r_body_work_frequency",
                "b_body_work_frequency",
                "r_leg_attack_frequency",
                "b_leg_attack_frequency",
                "r_recent_activity_level",
                "b_recent_activity_level",
                "r_form_consistency_alt",
                "b_form_consistency_alt",
                "form_consistency_alt_diff",
                "r_momentum_factor",
                "b_momentum_factor",
                "r_stamina_factor",
                "b_stamina_factor",
                "r_pressure_performance",
                "b_pressure_performance",
                "r_comeback_ability",
                "b_comeback_ability",
                "r_clutch_factor",
                "b_clutch_factor",
    ...
    def train_models(self, df):
        """Train stacked ensemble with NESTED CROSS-VALIDATION to prevent overfitting"""
        ...
        print(f"\n🔍 NESTED CROSS-VALIDATION: Starting with {len(feature_columns)} features")
        print("=" * 80)
        print("IMPLEMENTING NESTED CROSS-VALIDATION")
        print("=" * 80)
        print("Outer CV: 5 folds for model evaluation")
        print("Inner CV: 3 folds for feature selection")
        print("This prevents overfitting in feature selection phase")
        print("=" * 80)
        ...
        # Outer CV for model evaluation (5 folds)
        ...
        for fold, (train_idx, val_idx) in enumerate(outer_cv.split(X, y_winner)):
            print(f"\n📊 FOLD {fold + 1}/5")
            print("-" * 40)
            
            # Split data for this fold
            X_train_fold, X_val_fold = X.iloc[train_idx], X.iloc[val_idx]
            y_train_fold, y_val_fold = y_winner.iloc[train_idx], y_winner.iloc[val_idx]
            
            print(f"Training set: {len(X_train_fold)} samples")
            print(f"Validation set: {len(X_val_fold)} samples")
            
            # INNER CV: Feature selection on training set only
            print("🔍 Performing feature selection on training set...")
            
            # Use LassoCV for feature selection (3-fold inner CV)
            ...
            print(f"✅ Selected {len(selected_features)} features for fold {fold + 1}")
            
            # Apply same feature selection to validation set
            X_val_selected = X_val_fold[selected_features]
            
            # Train a simple model for this fold (XGBoost)
            from xgboost import XGBClassifier
            ...
            # Train and evaluate
            xgb_fold.fit(X_train_selected, y_train_fold)
            fold_score = xgb_fold.score(X_val_selected, y_val_fold)
        ...
        most_common_features = [feature for feature, count in feature_frequency.most_common()]
        
        print(f"\n🏆 MOST COMMONLY SELECTED FEATURES ACROSS FOLDS:")
        for i, feature in enumerate(most_common_features[:15]):
            freq = feature_frequency[feature]
            print(f"  {i+1:2d}. {feature:<30} (selected in {freq}/5 folds)")
        
        # Use the most commonly selected features for final model
        # Take features that were selected in at least 3 out of 5 folds
        final_features = [feature for feature, count in feature_frequency.items() if count >= 3]
        
        if len(final_features) < 10:  # Fallback if too few features
            final_features = most_common_features[:20]
        
        print(f"\n✅ FINAL FEATURE SELECTION: Using {len(final_features)} features")
        print("Features selected in at least 3/5 folds (or top 20 if fewer)")
        
        # Store the final feature selection
        self.original_feature_columns = feature_columns.copy()
        self.final_selected_features = final_features
        
        # Create a simple selector for prediction compatibility
        from sklearn.feature_selection import SelectKBest, f_classif
        self.feature_selector = SelectKBest(f_classif, k=len(final_features))
        
        # Fit on all data with final features
        X_final = X[final_features]
        self.feature_selector.fit(X_final, y_winner)
        
        print(f"\n🎯 PROCEEDING WITH FINAL MODEL TRAINING...")
        print(f"Using {len(final_features)} features selected by nested CV")
        ...
        # Use the final selected features from nested CV
        X_final = X[self.final_selected_features]
        
        # Store original indices for deep learning
        original_indices = X_final.index if hasattr(X_final, 'index') else np.arange(len(X_final))
        ...
        (
            ...
            X_final, y_winner, y_method, original_indices, test_size=0.2, random_state=42, stratify=y_winner
        )
        ...
        preprocessor = ColumnTransformer(
            [("num", numeric_transformer, self.final_selected_features)]
        )
        ...
        if self.use_deep_learning and HAS_TENSORFLOW:
            ...
            # Create DataFrames with selected features for preprocessor
            X_train_df = pd.DataFrame(X_train, columns=selected_features)
            X_test_df = pd.DataFrame(X_test, columns=selected_features)
            
            X_dl_train = [
                safe_get_values(df, train_indices, "r_fighter_encoded"),
                safe_get_values(df, train_indices, "b_fighter_encoded"),
                preprocessor.fit_transform(X_train_df),
            ]

            X_dl_test = [
                safe_get_values(df, test_indices, "r_fighter_encoded"),
                safe_get_values(df, test_indices, "b_fighter_encoded"),
            ...
            try:
                ...
                try:
                    ...
                    # Manual one-hot encoding if to_categorical not available
                    def to_categorical(y, num_classes):
                        return np.eye(num_classes)[y]

            # Safe method mapping with error handling
            try:
                ...
                print(f"Warning: Unknown method value {e} in data, using default encoding")
                # Use default encoding (all Decision) to prevent crash
                y_method_train_dl = to_categorical([2] * len(y_method_train), num_classes=6)
                y_method_test_dl = to_categorical([2] * len(y_method_test), num_classes=6)

            # Build and train deep learning model
            self.deep_learning_model = self.build_deep_learning_model(
                num_features=preprocessor.transform(X_train_df).shape[1],
        ...
        def train_fold(fold_data):
            ...
            fold, train_idx, val_idx, X, y_winner, preprocessor, selected_features = (
                fold_data
            )
            ...
            if not isinstance(X_fold_train, pd.DataFrame):
                X_fold_train = pd.DataFrame(X_fold_train, columns=selected_features)
            if not isinstance(X_fold_val, pd.DataFrame):
                X_fold_val = pd.DataFrame(X_fold_val, columns=selected_features)
            if not isinstance(y_fold_train, (pd.Series, np.ndarray)):
                y_fold_train = np.array(y_fold_train)
            if not isinstance(y_fold_val, (pd.Series, np.ndarray)):
                y_fold_val = np.array(y_fold_val)
        ...
        print(
            f"Test Set Accuracy: {self.winner_model.score(X_test_df, y_winner_test):.4f}"
        )
        print(f"{'=' * 80}\n")
    ...
    def prepare_upcoming_fight(self, fight, feature_columns):
        ...
        for prefix, stats in [("r", r_stats), ("b", b_stats)]:
            for stat in [
                "wins", "losses", "ko_rate", "sub_rate", "dec_rate",
                "pro_str_def", "pro_td_def", "durability", "pro_SLpM",
                "pro_sig_str_acc", "pro_SApM", "pro_td_avg", "pro_td_acc",
                "pro_sub_avg", "pro_kd_pM", "head_pct", "body_pct", "leg_pct",
                "distance_pct", "clinch_pct", "ground_pct", "fight_time_minutes",
                "recent_form", "recent_finish_rate", "win_streak", "loss_streak",
                "last_5_wins", "days_since_last_fight", "win_loss_ratio",
                "pro_total_str_pM", "pro_total_str_acc", "pro_total_str_absorbed_pM",
                "slpm_trend", "td_avg_trend", "age_adjusted_performance", "peak_indicator"
            ]:
                fight_features[f"{prefix}_{stat}_corrected"] = stats.get(stat, 0)

        # Create all the features that were present during training
        # This ensures feature consistency between training and prediction
        
        # Age and experience interaction
        fight_features["age_experience_interaction"] = fight_features["age_at_event_diff"] * (r_total_fights - b_total_fights) / 100.0
        
        # Championship pressure features
        r_championship_exp = 1 if r_total_fights > 0 and (r_stats.get("fight_time_minutes", 0) / r_total_fights) > 12 else 0
        b_championship_exp = 1 if b_total_fights > 0 and (b_stats.get("fight_time_minutes", 0) / b_total_fights) > 12 else 0
        fight_features["r_championship_pressure"] = r_championship_exp * fight_features["is_title_bout"] * r_stats.get("recent_form", 0)
        fight_features["b_championship_pressure"] = b_championship_exp * fight_features["is_title_bout"] * b_stats.get("recent_form", 0)
        
        # Clinch effectiveness features
        fight_features["r_clinch_effectiveness"] = r_stats.get("clinch_pct", 0) * r_stats.get("pro_SLpM", 0) * r_stats.get("pro_sig_str_acc", 0)
        fight_features["b_clinch_effectiveness"] = b_stats.get("clinch_pct", 0) * b_stats.get("pro_SLpM", 0) * b_stats.get("pro_sig_str_acc", 0)
        
        # Knockdown resistance features
        fight_features["r_kd_resistance"] = 1 - (r_stats.get("pro_SApM", 0) / 6.0) * (1 - r_stats.get("pro_str_def", 0))
        fight_features["b_kd_resistance"] = 1 - (b_stats.get("pro_SApM", 0) / 6.0) * (1 - b_stats.get("pro_str_def", 0))
        
        # Power strike ratio features
        fight_features["r_power_strike_ratio"] = r_stats.get("pro_kd_pM", 0) / (r_stats.get("pro_SLpM", 0) + 0.001)
        fight_features["b_power_strike_ratio"] = b_stats.get("pro_kd_pM", 0) / (b_stats.get("pro_SLpM", 0) + 0.001)
        
        # Create all the comprehensive features that were present during training
        # This is a comprehensive list of all features that might be needed
        
        # Core derived features
        fight_features["net_striking_advantage"] = (
            fight_features["r_pro_SLpM_corrected"] - fight_features["r_pro_SApM_corrected"]
        ) - (fight_features["b_pro_SLpM_corrected"] - fight_features["b_pro_SApM_corrected"])

        fight_features["striking_efficiency"] = (
            fight_features["r_pro_SLpM_corrected"] * fight_features["r_pro_sig_str_acc_corrected"]
        ) - (fight_features["b_pro_SLpM_corrected"] * fight_features["b_pro_sig_str_acc_corrected"])

        fight_features["defensive_striking"] = (
            fight_features["r_pro_str_def_corrected"] - fight_features["r_pro_SApM_corrected"]
        ) - (fight_features["b_pro_str_def_corrected"] - fight_features["b_pro_SApM_corrected"])

        fight_features["grappling_control"] = (
            fight_features["r_pro_td_avg_corrected"] * fight_features["r_pro_td_acc_corrected"]
        ) - (fight_features["b_pro_td_avg_corrected"] * fight_features["b_pro_td_acc_corrected"])

        fight_features["grappling_defense"] = (
            fight_features["r_pro_td_def_corrected"] - fight_features["r_pro_sub_avg_corrected"] / 5
        ) - (fight_features["b_pro_td_def_corrected"] - fight_features["b_pro_sub_avg_corrected"] / 5)

        fight_features["offensive_output"] = (
            fight_features["r_pro_SLpM_corrected"]
            + fight_features["r_pro_td_avg_corrected"]
            + fight_features["r_pro_sub_avg_corrected"]
        ) - (
            fight_features["b_pro_SLpM_corrected"]
            + fight_features["b_pro_td_avg_corrected"]
            + fight_features["b_pro_sub_avg_corrected"]
        )

        fight_features["defensive_composite"] = (
            (fight_features["r_pro_str_def_corrected"] + fight_features["r_pro_td_def_corrected"]) / 2
        ) - ((fight_features["b_pro_str_def_corrected"] + fight_features["b_pro_td_def_corrected"]) / 2)

        fight_features["ko_specialist_gap"] = (
            fight_features["r_ko_rate_corrected"] * fight_features["r_pro_SLpM_corrected"]
        ) - (fight_features["b_ko_rate_corrected"] * fight_features["b_pro_SLpM_corrected"])

        # Ensure ALL features from training are present
        # This is critical for feature consistency between training and prediction
        # Use final selected features from nested CV
        final_features = getattr(self, 'final_selected_features', feature_columns)
        for feature in final_features:
            if feature not in fight_features:
                fight_features[feature] = 0.0
    ...
    def predict_fight(self, fight_data, feature_columns):
        ...
        X = fight_data[final_features]
        
        # Apply feature selection if available
        if hasattr(self, 'feature_selector'):
            # Transform and convert back to DataFrame to maintain column names
            X_transformed = self.feature_selector.transform(X)
            # Get the selected feature names
            selected_features = [final_features[i] for i in self.feature_selector.get_support(indices=True)]
            # Convert back to DataFrame with proper column names
            X = pd.DataFrame(X_transformed, columns=selected_features, index=X.index)
        ...
        if self.use_deep_learning and self.deep_learning_model and self.fighter_encoder:
            try:
                ...
                if (
                    ...
                    # Get the selected features for the preprocessor with robust fallback
                    if hasattr(self.feature_selector, 'get_feature_names_out'):
                        selected_features_for_dl = self.feature_selector.get_feature_names_out()
                    elif hasattr(self.feature_selector, 'get_support'):
                        selected_features_for_dl = [feature_columns[i] for i in self.feature_selector.get_support(indices=True)]
                    else:
                        selected_features_for_dl = feature_columns
                    preprocessor = ColumnTransformer(
                        [("num", numeric_transformer, selected_features_for_dl)]
                    )
                    # Create DataFrame with correct column names for preprocessor
                    X_df = pd.DataFrame(X, columns=selected_features_for_dl)
                    X_scaled = preprocessor.fit_transform(X_df)
    ...
    def predict_upcoming_fights(self, upcoming_fights, feature_columns):
        ...
        for fight in upcoming_fights:
            ...
            # Ensure all required features are present
            # Use the feature columns that were selected during training
            for col in feature_columns:
