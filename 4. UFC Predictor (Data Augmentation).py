...
class AdvancedUFCPredictor:
    ...
    def augment_data_with_corner_swapping(self, df):
        """Augment data by swapping red and blue corners to eliminate bias"""
        print("Augmenting data with corner swapping to eliminate red corner bias...")
        ...
        if new_total_count > 0:
            ...
            print(f"After augmentation - Red: {new_red_bias:.3f}, Blue: {1-new_red_bias:.3f}")
            print(f"Dataset size: {len(df)} -> {len(df_combined)} (doubled with corner swapping)")
        
        # Set balanced class weights for the augmented dataset
        self.class_weight = {0: 0.5, 1: 0.5}  # Balanced weights since bias is eliminated
    ...
    def swap_corners(self, df):
        ...
        df_swapped['r_fighter'], df_swapped['b_fighter'] = df_swapped['b_fighter'], df_swapped['r_fighter']
        if 'r_name' in df_swapped.columns and 'b_name' in df_swapped.columns:
            df_swapped['r_name'], df_swapped['b_name'] = df_swapped['b_name'], df_swapped['r_name']
        ...
        red_columns = [col for col in df.columns if col.startswith('r_') and col not in ['r_fighter', 'r_name']]
        blue_columns = [col for col in df.columns if col.startswith('b_') and col not in ['b_fighter', 'b_name']]
        ...
        diff_columns = [col for col in df.columns if col.endswith('_diff_corrected') or col.endswith('_diff')]
        ...
        derived_features_to_negate = [
            ...
            'finish_threat', 'momentum_advantage', 'pace_differential', 'avg_fight_time_diff',
            ...
            'ko_specialist_matchup', 'sub_specialist_matchup', 'durability_advantage',
            'kd_resistance_advantage', 'striker_vs_grappler', 'distance_ground_preference',
            'clinch_advantage', 'head_hunting_advantage', 'vulnerability_advantage',
            ...
            'championship_pressure_advantage', 'opponent_quality_advantage', 'technical_striker_advantage',
            'clinch_effectiveness_advantage', 'momentum_velocity', 'championship_impact',
            'opponent_quality_momentum', 'finishing_pressure_stress', 'ring_rust_vs_momentum',
            'weight_class_adaptation', 'stance_versatility_impact', 'power_vs_technique',
            'cardio_advantage', 'finish_pressure', 'opponent_quality_gap', 'recent_opponent_strength',
            'upset_potential'
        ]
        ...
        advantage_columns = [col for col in df.columns if col.endswith('_advantage') or col.endswith('_better')]
        ...
        if 'orthodox_southpaw_matchup' in df_swapped.columns:
            df_swapped['orthodox_southpaw_matchup'] = -df_swapped['orthodox_southpaw_matchup']
    ...
    def prepare_features(self, df):
        ...
        df["style_matchup_depth"] = (
            abs(df["striker_advantage"]) + abs(df["grappler_advantage"]) + 
        ...
        df["ring_rust_vs_momentum"] = (
            df["ring_rust_factor"] * (df["r_recent_form_corrected"] - df["b_recent_form_corrected"])
        )
        ...
        df["weight_class_adaptation"] = (
            df["weight_class_factor"] * (df["r_pro_SLpM_corrected"] - df["b_pro_SLpM_corrected"])
        )
        ...
        feature_columns.extend([
            ...
            "momentum_velocity", "style_matchup_depth", "championship_impact", "opponent_quality_momentum",
            "finishing_pressure_stress", "ring_rust_vs_momentum", "weight_class_adaptation", "stance_versatility_impact"
        ])
    ...
    def train_models(self, df):
        ...
        if HAS_XGBOOST:
            ...
            xgb_params = {
                'n_estimators': 800, 'max_depth': 10, 'learning_rate': 0.015,  # Match Class Weighting parameters
                'subsample': 0.85, 'colsample_bytree': 0.85, 'colsample_bylevel': 0.85,  # Match Class Weighting
                'n_jobs':-1, 'reg_alpha': 0.1, 'reg_lambda': 0.8, 'min_child_weight': 3,  # Match Class Weighting
                'gamma': 0.15, 'scale_pos_weight': scale_pos, 'random_state': 42,  # Match Class Weighting
                'eval_metric': "logloss", 'tree_method': 'hist', 'seed': 42,
                'enable_categorical': True, 'max_delta_step': 1  # Better for imbalanced data
            }
            ...
            try:
                ...
                xgb_classifier = XGBClassifier(
                    ...
                    reg_alpha=0.1, reg_lambda=0.8, min_child_weight=3,  # Match Class Weighting
            ...
            xgb_model = Pipeline([
                ...
                ("feature_selector", SelectPercentile(f_classif, percentile=85)),  # Match Class Weighting
        ...
        if self.use_ensemble and len(base_models) > 1:
            self.winner_model = StackingClassifier(
                ...
                cv=10,  # Increased CV folds for better generalization
                n_jobs=-1,  # Use all CPU cores for faster training (causes multiple windows in .exe)
        ...
        tscv = TimeSeriesSplit(n_splits=7)  # Match Class Weighting
