...
class AdvancedUFCPredictor:
    ...
    def fix_red_corner_bias(self, df):
        ...
        if total_count > 0:
            ...
            self.class_weight = {
                0: red_count
                / total_count,  # Blue weight = red frequency (counteract red bias)
                1: blue_count
                / total_count,  # Red weight = blue frequency (counteract blue bias)
            }
            print(
                f"Class weights - Blue: {self.class_weight[0]:.3f}, Red: {self.class_weight[1]:.3f}"
            )
            print(
                f"Bias correction: Blue gets {self.class_weight[0] / self.class_weight[1]:.2f}x more weight"
            )
        else:
