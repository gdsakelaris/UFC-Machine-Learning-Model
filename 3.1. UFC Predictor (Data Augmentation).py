...
class AdvancedUFCPredictor:
    ...
    def calculate_career_stage(self, df, fighter_col):
        ...
        Uses age-first approach with prime years 27-34 and experience considerations"""
        stages = []
        for idx, row in df.iterrows():
            fighter = row[fighter_col]
            if pd.isna(fighter):
            ...
            # More logical career stage calculation
            if total_fights <= 3:
                stage = 0  # Rookie
            elif total_fights <= 8:
                stage = 1  # Developing
            elif 27 <= age <= 34 and total_fights >= 9:
                stage = 2  # Prime
            elif 35 <= age <= 38 and total_fights >= 9:
                stage = 3  # Declining
            elif total_fights >= 25 or age >= 39:
                stage = 4  # Veteran
            else:
                stage = 1  # Default to developing for edge cases
    ...
    def calculate_event_significance(self, df):
        """Calculate event significance factor based on 3-digit UFC event numbers"""
        import re
        significances = []
        for idx, row in df.iterrows():
            ...
            # Event number factor based on 3-digit UFC event numbers
            event_factor = 1.0
            if 'UFC' in str(event_name).upper():
                # Look for 3-digit number pattern (e.g., "UFC 319", "UFC 300")
                ufc_number_match = re.search(r'UFC\s+(\d{3,})', str(event_name).upper())
                if ufc_number_match:
                    event_number = int(ufc_number_match.group(1))
                    # Higher numbered events are generally more significant
                    if event_number >= 300:  # Major milestone events
                        event_factor = 1.3
                    elif event_number >= 200:  # Established numbered events
                        event_factor = 1.2
                    elif event_number >= 100:  # Early numbered events
                        event_factor = 1.1
                    else:  # Very early events
                        event_factor = 1.0
                else:
                    # UFC event without 3-digit number (Fight Night, etc.)
                    event_factor = 0.9
            else:
                # Non-UFC events
                event_factor = 0.8
            
            significance = title_factor * rounds_factor * event_factor
...
class UFCPredictorGUI:
    def __init__(self, root):
        ...
        self.output_file_path = tk.StringVar(value="UFC_predictions_3_1.xlsx")
