...
class AdvancedUFCPredictor:
    ...
    def prepare_features(self, df):
        ...
        # Create corrected column mappings for backward compatibility
        corrected_mappings = {
            'r_pro_SLpM_corrected': 'r_pro_SLpM',
            'b_pro_SLpM_corrected': 'b_pro_SLpM',
            'r_pro_SApM_corrected': 'r_pro_SApM',
            'b_pro_SApM_corrected': 'b_pro_SApM',
            'r_pro_sig_str_acc_corrected': 'r_pro_sig_str_acc',
            'b_pro_sig_str_acc_corrected': 'b_pro_sig_str_acc',
            'r_pro_str_def_corrected': 'r_pro_str_def',
            'b_pro_str_def_corrected': 'b_pro_str_def',
            'r_pro_td_avg_corrected': 'r_pro_td_avg',
            'b_pro_td_avg_corrected': 'b_pro_td_avg',
            'r_pro_td_acc_corrected': 'r_pro_td_acc',
            'b_pro_td_acc_corrected': 'b_pro_td_acc',
            'r_pro_td_def_corrected': 'r_pro_td_def',
            'b_pro_td_def_corrected': 'b_pro_td_def',
            'r_pro_sub_avg_corrected': 'r_pro_sub_avg',
            'b_pro_sub_avg_corrected': 'b_pro_sub_avg',
            'r_pro_kd_pM_corrected': 'r_pro_kd_pM',
            'b_pro_kd_pM_corrected': 'b_pro_kd_pM',
            'r_ko_rate_corrected': 'r_ko_rate',
            'b_ko_rate_corrected': 'b_ko_rate',
            'r_sub_rate_corrected': 'r_sub_rate',
            'b_sub_rate_corrected': 'b_sub_rate',
            'r_dec_rate_corrected': 'r_dec_rate',
            'b_dec_rate_corrected': 'b_dec_rate',
            'r_head_pct_corrected': 'r_head_pct',
            'b_head_pct_corrected': 'b_head_pct',
            'r_body_pct_corrected': 'r_body_pct',
            'b_body_pct_corrected': 'b_body_pct',
            'r_leg_pct_corrected': 'r_leg_pct',
            'b_leg_pct_corrected': 'b_leg_pct',
            'r_distance_pct_corrected': 'r_distance_pct',
            'b_distance_pct_corrected': 'b_distance_pct',
            'r_clinch_pct_corrected': 'r_clinch_pct',
            'b_clinch_pct_corrected': 'b_clinch_pct',
            'r_ground_pct_corrected': 'r_ground_pct',
            'b_ground_pct_corrected': 'b_ground_pct'
        }
        
        # Create the corrected columns if they don't exist
        for corrected_col, original_col in corrected_mappings.items():
            if original_col in df.columns and corrected_col not in df.columns:
                df[corrected_col] = df[original_col]
        
        # Create missing rate columns
        if 'r_ko_rate' not in df.columns:
            df['r_ko_rate'] = df['r_wins'] / (df['r_wins'] + df['r_losses'] + 1e-6)
            df['b_ko_rate'] = df['b_wins'] / (df['b_wins'] + df['b_losses'] + 1e-6)
            df['r_sub_rate'] = 0.1  # Default value
            df['b_sub_rate'] = 0.1  # Default value
            df['r_dec_rate'] = 1 - df['r_ko_rate'] - df['r_sub_rate']
            df['b_dec_rate'] = 1 - df['b_ko_rate'] - df['b_sub_rate']
        
        # Create missing kd_pM columns
        if 'r_pro_kd_pM' not in df.columns:
            df['r_pro_kd_pM'] = 0.1  # Default value
            df['b_pro_kd_pM'] = 0.1  # Default value
        
        # Create corrected versions of rate columns
        df['r_ko_rate_corrected'] = df['r_ko_rate']
        df['b_ko_rate_corrected'] = df['b_ko_rate']
        df['r_sub_rate_corrected'] = df['r_sub_rate']
        df['b_sub_rate_corrected'] = df['b_sub_rate']
        df['r_dec_rate_corrected'] = df['r_dec_rate']
        df['b_dec_rate_corrected'] = df['b_dec_rate']
        
        # Create all other corrected columns that might be missing
        corrected_columns = [
            'r_wins_corrected', 'b_wins_corrected', 'r_losses_corrected', 'b_losses_corrected',
            'r_win_loss_ratio_corrected', 'b_win_loss_ratio_corrected', 'r_durability_corrected', 'b_durability_corrected',
            'r_days_since_last_fight_corrected', 'b_days_since_last_fight_corrected',
            'r_fight_time_minutes_corrected', 'b_fight_time_minutes_corrected',
            'r_head_pct_corrected', 'b_head_pct_corrected', 'r_body_pct_corrected', 'b_body_pct_corrected',
            'r_leg_pct_corrected', 'b_leg_pct_corrected', 'r_distance_pct_corrected', 'b_distance_pct_corrected',
            'r_clinch_pct_corrected', 'b_clinch_pct_corrected', 'r_ground_pct_corrected', 'b_ground_pct_corrected',
            'r_win_streak_corrected', 'b_win_streak_corrected', 'r_loss_streak_corrected', 'b_loss_streak_corrected',
            'r_last_5_wins_corrected', 'b_last_5_wins_corrected', 'r_recent_finish_rate_corrected', 'b_recent_finish_rate_corrected',
            'r_slpm_trend_corrected', 'b_slpm_trend_corrected', 'r_td_avg_trend_corrected', 'b_td_avg_trend_corrected',
            ...
            # Diff corrected columns
            'pro_SLpM_diff_corrected', 'pro_sig_str_acc_diff_corrected', 'pro_SApM_diff_corrected', 'pro_str_def_diff_corrected',
            'pro_td_avg_diff_corrected', 'pro_td_acc_diff_corrected', 'pro_td_def_diff_corrected', 'pro_sub_avg_diff_corrected',
            'pro_kd_pM_diff_corrected', 'ko_rate_diff_corrected', 'sub_rate_diff_corrected', 'dec_rate_diff_corrected',
            'recent_form_diff_corrected', 'head_pct_diff_corrected', 'body_pct_diff_corrected', 'leg_pct_diff_corrected',
            'distance_pct_diff_corrected', 'clinch_pct_diff_corrected', 'ground_pct_diff_corrected',
            'win_streak_diff_corrected', 'loss_streak_diff_corrected', 'last_5_wins_diff_corrected',
            'days_since_last_fight_diff_corrected', 'recent_finish_rate_diff_corrected', 'durability_diff_corrected',
            'slpm_trend_diff_corrected', 'td_avg_trend_diff_corrected', 'age_adjusted_performance_diff_corrected',
        ...
        for col in corrected_columns:
            if col not in df.columns:
                ...
                base_col = col.replace('_corrected', '')
    ...
    def train_models(self, df):
        ...
        # Apply bias correction through class weighting (simpler approach)
        print("\n" + "=" * 80)
        print("IMPLEMENTING BIAS CORRECTION THROUGH CLASS WEIGHTING")
        print("=" * 80)
        
        # Calculate current bias
        red_wins = df[df["winner"] == "Red"].shape[0]
        blue_wins = df[df["winner"] == "Blue"].shape[0]
        total_fights = df.shape[0]
        red_bias = (red_wins - blue_wins) / total_fights
        print(f"Original bias: Red corner advantage of {red_bias:.1%}")
        print(f"Red corner wins: {red_wins} ({red_wins/total_fights:.1%})")
        print(f"Blue corner wins: {blue_wins} ({blue_wins/total_fights:.1%})")
        print("Using class weighting to address bias...")
        ...
        if HAS_XGBOOST:
            print("\n✓ XGBoost available")
            # Calculate class weights to address bias
            ...
            scale_pos = class_weight_dict[0] / class_weight_dict[1]  # Blue class weight / Red class weight
            
            print(f"Class weights: {class_weight_dict}")
            print(f"Scale pos weight: {scale_pos:.3f}")
            xgb_model = Pipeline([
        ...
        rf_model = Pipeline([
            ...
            ("classifier", RandomForestClassifier(
                ...
                class_weight='balanced'  # Add class weighting to address bias
            ))
        ])
        ...
        if HAS_XGBOOST:
            # Calculate class weights for method prediction
            from sklearn.utils.class_weight import compute_class_weight
            method_classes = np.unique(y_method_train)
            method_class_weights = compute_class_weight('balanced', classes=method_classes, y=y_method_train)
            method_class_weight_dict = dict(zip(method_classes, method_class_weights))
            
            print(f"Method class weights: {method_class_weight_dict}")
            ...
            xgb_method = Pipeline([
                ...
                ("classifier", XGBClassifier(
                    ...
                    tree_method='hist',  # Use histogram method for consistency
                    seed=42,  # Additional XGBoost seed
                    class_weight=method_class_weight_dict  # Add class weighting for method prediction
                ))
            ])
