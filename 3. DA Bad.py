...
class AdvancedUFCPredictor:
    ...
    def augment_data_with_corner_swapping(self, df):
        ...
        if total_count > 0:
            ...
            bias_info = f"Original bias - Red: {red_bias:.3f}, Blue: {1 - red_bias:.3f}"
            print(bias_info)
            # Add to debug results for GUI users
            if hasattr(self, 'debug_results'):
                self.debug_results.append("")
                self.debug_results.append("🔄 DATA AUGMENTATION ANALYSIS:")
                self.debug_results.append("-" * 40)
                self.debug_results.append(bias_info)
            else:
                # Initialize debug_results if it doesn't exist
                self.debug_results = []
                self.debug_results.append("")
                self.debug_results.append("🔄 DATA AUGMENTATION ANALYSIS:")
                self.debug_results.append("-" * 40)
        ...
        if abs(red_bias - 0.5) > bias_threshold:
            bias_detection = f"Significant bias detected ({abs(red_bias - 0.5):.3f}), applying enhanced augmentation..."
            print(bias_detection)
            if hasattr(self, 'debug_results'):
                self.debug_results.append(bias_detection)
            ...
            augmentation_result = f"Enhanced augmentation: {len(df)} -> {len(df_combined)} (1.75x size)"
            print(augmentation_result)
            if hasattr(self, 'debug_results'):
                self.debug_results.append(augmentation_result)
        else:
            light_bias_msg = "Bias is minimal, applying light augmentation for method prediction..."
            print(light_bias_msg)
            if hasattr(self, 'debug_results'):
                self.debug_results.append(light_bias_msg)
            # Light augmentation even with minimal bias for method prediction
            df_augmented = self.swap_corners(df)
            sample_size = len(df_augmented) // 4  # 25% augmentation
            df_augmented_sampled = df_augmented.sample(n=sample_size, random_state=42)
            df_combined = pd.concat([df, df_augmented_sampled], ignore_index=True)
            light_result = f"Light augmentation: {len(df)} -> {len(df_combined)} (1.25x size)"
            print(light_result)
            if hasattr(self, 'debug_results'):
                self.debug_results.append(light_result)
        ...
        if new_total_count > 0:
            ...
            final_bias = f"After smart augmentation - Red: {new_red_bias:.3f}, Blue: {1 - new_red_bias:.3f}"
            print(final_bias)
            if hasattr(self, 'debug_results'):
                self.debug_results.append(final_bias)
        ...
        # Save updated debug results to file
        if hasattr(self, 'debug_results'):
            with open("Debugging_results.txt", "w", encoding="utf-8") as f:
                for line in self.debug_results:
                    f.write(line + "\n")
    ...
    def prepare_features(self, df):
        ...
        debug_results = []
        debug_results.append("=" * 60)
        debug_results.append("FEATURE DEBUGGING REPORT")
        debug_results.append("=" * 60)
        debug_results.append(f"DataFrame shape: {df.shape}")
        debug_results.append("")
        
        # Track feature creation success/failure
        feature_status = {}
        
        # Initialize missing critical dependencies list
        missing_critical = []
        
        # COMPREHENSIVE FEATURE TRACKING WRAPPER
        def track_feature_creation(feature_name, creation_func, *args, **kwargs):
            """Wrapper to track all feature creation attempts"""
            try:
                result = creation_func(*args, **kwargs)
                if feature_name in df.columns:
                    # Check if feature has meaningful values
                    if df[feature_name].dtype in ['object', 'datetime64[ns]']:
                        feature_status[feature_name] = "SUCCESS"
                    elif df[feature_name].sum() == 0:
                        feature_status[feature_name] = "ZERO_VALUES"
                    else:
                        feature_status[feature_name] = "SUCCESS"
                else:
                    feature_status[feature_name] = "NOT_CREATED"
                return result
            except Exception as e:
                debug_results.append(f"❌ ERROR: {feature_name} creation failed: {e}")
                feature_status[feature_name] = f"ERROR: {str(e)[:50]}"
                return None
        
        # Track existing features before any modifications
        initial_features = set(df.columns)
        debug_results.append(f"🔍 INITIAL FEATURES: {len(initial_features)}")
        ...
        if cache_key in self.feature_cache:
            ...
            # Even with cached features, save basic debug info
            debug_results.append("")
            debug_results.append("⚠️ USING CACHED FEATURES - Limited debug info available")
            debug_results.append(f"DataFrame shape: {df.shape}")
            debug_results.append(f"Total features: {len(df.columns)}")
            with open("Debugging_results.txt", "w", encoding="utf-8") as f:
                for line in debug_results:
                    f.write(line + "\n")
        ...
        # ============================================================================
        # NEW HIGH-IMPACT WINNER PREDICTION FEATURES (8 features)
        # ============================================================================
        ...
        try:
            ...
            feature_status["stamina_factor_diff"] = "SUCCESS"
        except Exception as e:
            debug_results.append(f"❌ ERROR: stamina_factor_diff creation failed: {e}")
            df["stamina_factor_diff"] = 0
            feature_status["stamina_factor_diff"] = "ERROR_FALLBACK_TO_ZERO"
        ...
        try:
            ...
            feature_status["pressure_performance_diff"] = "SUCCESS"
        except Exception as e:
            debug_results.append(f"❌ ERROR: pressure_performance_diff creation failed: {e}")
            df["pressure_performance_diff"] = 0
            feature_status["pressure_performance_diff"] = "ERROR_FALLBACK_TO_ZERO"
        ...
        # ============================================================================
        # 26 NEW HIGH-IMPACT DIFFERENTIAL FEATURES
        # ============================================================================

        # Enhanced feature engineering with better error handling
        def safe_divide(numerator, denominator, default=0.0):
            """Safely divide with fallback for zero/NaN values"""
            return np.where(
        ...
        # POWER VS VOLUME ANALYSIS
        df["r_power_strike_ratio"] = safe_divide(df["r_pro_kd_pM_corrected"], df["r_pro_SLpM_corrected"])
        df["b_power_strike_ratio"] = safe_divide(df["b_pro_kd_pM_corrected"], df["b_pro_SLpM_corrected"])
        df["power_strike_ratio_diff"] = df["r_power_strike_ratio"] - df["b_power_strike_ratio"]
        
        # STRIKE EFFICIENCY GAP
        df["r_strike_efficiency"] = df["r_pro_sig_str_acc_corrected"] - df["r_pro_SApM_corrected"]
        df["b_strike_efficiency"] = df["b_pro_sig_str_acc_corrected"] - df["b_pro_SApM_corrected"]
        df["strike_efficiency_gap"] = df["r_strike_efficiency"] - df["b_strike_efficiency"]
        
        # DAMAGE RATIO
        df["r_damage_ratio"] = safe_divide(df["r_pro_SLpM_corrected"], df["r_pro_SApM_corrected"])
        df["b_damage_ratio"] = safe_divide(df["b_pro_SLpM_corrected"], df["b_pro_SApM_corrected"])
        df["damage_ratio_diff"] = df["r_damage_ratio"] - df["b_damage_ratio"]
        
        # TAKEDOWN EFFICIENCY
        df["r_takedown_efficiency"] = df["r_pro_td_acc_corrected"] * df["r_pro_td_avg_corrected"]
        df["b_takedown_efficiency"] = df["b_pro_td_acc_corrected"] * df["b_pro_td_avg_corrected"]
        df["takedown_efficiency_diff"] = df["r_takedown_efficiency"] - df["b_takedown_efficiency"]
        
        # GROUND CONTROL RATIO
        df["r_ground_control_ratio"] = safe_divide(df["r_pro_td_avg_corrected"], df["r_pro_sub_avg_corrected"])
        df["b_ground_control_ratio"] = safe_divide(df["b_pro_td_avg_corrected"], df["b_pro_sub_avg_corrected"])
        df["ground_control_ratio_diff"] = df["r_ground_control_ratio"] - df["b_ground_control_ratio"]
        
        # GRAPPLING BALANCE
        df["r_grappling_balance"] = (df["r_pro_td_avg_corrected"] + df["r_pro_sub_avg_corrected"]) / 2
        df["b_grappling_balance"] = (df["b_pro_td_avg_corrected"] + df["b_pro_sub_avg_corrected"]) / 2
        df["grappling_balance_diff"] = df["r_grappling_balance"] - df["b_grappling_balance"]
        
        # DEFENSIVE RATIO
        df["r_defensive_ratio"] = safe_divide(df["r_pro_str_def_corrected"], df["r_pro_SApM_corrected"])
        df["b_defensive_ratio"] = safe_divide(df["b_pro_str_def_corrected"], df["b_pro_SApM_corrected"])
        df["defensive_ratio_diff"] = df["r_defensive_ratio"] - df["b_defensive_ratio"]
        
        # TAKEDOWN DEFENSE STRENGTH
        df["r_takedown_defense_strength"] = df["r_pro_td_def_corrected"] * (1 - df["r_pro_SApM_corrected"])
        df["b_takedown_defense_strength"] = df["b_pro_td_def_corrected"] * (1 - df["b_pro_SApM_corrected"])
        df["takedown_defense_strength_diff"] = df["r_takedown_defense_strength"] - df["b_takedown_defense_strength"]
        
        # OVERALL DEFENSE SCORE
        df["r_overall_defense_score"] = (df["r_pro_str_def_corrected"] + df["r_pro_td_def_corrected"]) / 2
        df["b_overall_defense_score"] = (df["b_pro_str_def_corrected"] + df["b_pro_td_def_corrected"]) / 2
        df["overall_defense_score_diff"] = df["r_overall_defense_score"] - df["b_overall_defense_score"]
        
        # EXPERIENCE FACTOR
        df["r_experience_factor"] = safe_divide(df["r_total_fights"], df["r_age_at_event"])
        df["b_experience_factor"] = safe_divide(df["b_total_fights"], df["b_age_at_event"])
        df["experience_factor_diff"] = df["r_experience_factor"] - df["b_experience_factor"]
        
        # WIN RATE CONSISTENCY
        df["r_win_rate_consistency"] = safe_divide(df["r_wins"], df["r_wins"] + df["r_losses"])
        df["b_win_rate_consistency"] = safe_divide(df["b_wins"], df["b_wins"] + df["b_losses"])
        df["win_rate_consistency_diff"] = df["r_win_rate_consistency"] - df["b_win_rate_consistency"]

        # ============================================================================
        # REMAINING 15 HIGH-IMPACT FEATURES
        # ============================================================================

        # FINISH RATE DIFFERENTIAL
        ...
        # STANCE ADVANTAGE (use existing stance_matchup_advantage)
        # Ensure stance_matchup_advantage is properly created
        if "stance_matchup_advantage" not in df.columns:
            debug_results.append("WARNING: stance_matchup_advantage not found, creating with default value")
            df["stance_matchup_advantage"] = 0
            feature_status["stance_matchup_advantage"] = "MISSING_CREATED_ZERO"
        else:
            # Check if stance features have meaningful values
            stance_sum = df["stance_matchup_advantage"].sum()
            if stance_sum == 0:
                debug_results.append("WARNING: stance_matchup_advantage has all zero values")
                feature_status["stance_matchup_advantage"] = "ZERO_VALUES"
            else:
                debug_results.append(f"SUCCESS: stance_matchup_advantage sum = {stance_sum}")
                feature_status["stance_matchup_advantage"] = "SUCCESS"
        
        df["stance_advantage"] = df["stance_matchup_advantage"]
        
        # Add missing high-impact features
        if "stance_versatility_advantage" not in df.columns:
            df["stance_versatility_advantage"] = np.where(
                df["r_stance"] == "Switch", 0.14,
                np.where(df["b_stance"] == "Switch", -0.14, 0)
            )
        
        if "orthodox_vs_switch" not in df.columns:
            df["orthodox_vs_switch"] = np.where(
                (df["r_stance"] == "Orthodox") & (df["b_stance"] == "Switch"), 0.15,
                np.where((df["r_stance"] == "Switch") & (df["b_stance"] == "Orthodox"), -0.15, 0)
            )
        
        if "southpaw_vs_switch" not in df.columns:
            df["southpaw_vs_switch"] = np.where(
                (df["r_stance"] == "Southpaw") & (df["b_stance"] == "Switch"), 0.13,
                np.where((df["r_stance"] == "Switch") & (df["b_stance"] == "Southpaw"), -0.13, 0)
            )
        
        if "orthodox_vs_southpaw" not in df.columns:
            df["orthodox_vs_southpaw"] = np.where(
                (df["r_stance"] == "Orthodox") & (df["b_stance"] == "Southpaw"), -0.06,
                np.where((df["r_stance"] == "Southpaw") & (df["b_stance"] == "Orthodox"), 0.06, 0)
            )
        
        if "reach_utilization_diff" not in df.columns:
            df["reach_utilization_diff"] = safe_divide(
                df["r_reach"] - df["b_reach"], 
                (df["r_reach"] + df["b_reach"]) / 2, 
                0
            )
        
        if "size_advantage_factor" not in df.columns:
            df["size_advantage_factor"] = (
                (df["r_height"] - df["b_height"]) * 0.3 + 
                (df["r_reach"] - df["b_reach"]) * 0.7
            ) / 100
        
        if "title_fight_experience_diff" not in df.columns:
            df["title_fight_experience_diff"] = (
                df["r_championship_experience"] - df["b_championship_experience"]
            )
        
        if "head_hunting_ratio_diff" not in df.columns:
            r_head_ratio = safe_divide(df["r_head"], df["r_sig_str"], 0)
            b_head_ratio = safe_divide(df["b_head"], df["b_sig_str"], 0)
            df["head_hunting_ratio_diff"] = r_head_ratio - b_head_ratio
        
        if "body_work_frequency_diff" not in df.columns:
            r_body_ratio = safe_divide(df["r_body"], df["r_sig_str"], 0)
            b_body_ratio = safe_divide(df["b_body"], df["b_sig_str"], 0)
            df["body_work_frequency_diff"] = r_body_ratio - b_body_ratio
        
        if "leg_attack_frequency_diff" not in df.columns:
            r_leg_ratio = safe_divide(df["r_leg"], df["r_sig_str"], 0)
            b_leg_ratio = safe_divide(df["b_leg"], df["b_sig_str"], 0)
            df["leg_attack_frequency_diff"] = r_leg_ratio - b_leg_ratio
        
        if "recent_activity_level_diff" not in df.columns:
            df["recent_activity_level_diff"] = (
                df["r_days_since_last_fight_corrected"] - 
                df["b_days_since_last_fight_corrected"]
            ) / 30
        
        if "form_consistency_alt_diff" not in df.columns:
            df["form_consistency_alt_diff"] = (
                df["r_recent_form_corrected"] - df["b_recent_form_corrected"]
            ) * 0.5
        
        if "momentum_factor_diff" not in df.columns:
            df["momentum_factor_diff"] = (
                df["r_momentum_score"] - df["b_momentum_score"]
            ) * 0.3
        
        # COMPREHENSIVE FEATURE ANALYSIS
        debug_results.append("")
        debug_results.append("=" * 60)
        debug_results.append("FEATURE ANALYSIS")
        debug_results.append("=" * 60)
        
        # Get ALL features in the model
        all_model_features = [col for col in df.columns if col not in ['winner', 'method', 'round', 'time']]
        final_features = set(df.columns)
        new_features = final_features - initial_features
        
        debug_results.append(f"TOTAL FEATURES: {len(all_model_features)}")
        debug_results.append(f"INITIAL FEATURES: {len(initial_features)}")
        debug_results.append(f"NEW FEATURES CREATED: {len(new_features)}")
        debug_results.append("")
        ...
        for feature in all_model_features:
            if feature not in feature_status:
                try:
                    if df[feature].dtype in ['object', 'datetime64[ns]']:
                        feature_status[feature] = "SUCCESS (Categorical/DateTime)"
                    elif df[feature].sum() == 0:
                        feature_status[feature] = "ZERO_VALUES"
                    elif df[feature].isna().all():
                        feature_status[feature] = "ALL_NAN"
                    else:
                        feature_status[feature] = "SUCCESS"
                except Exception as e:
                    feature_status[feature] = f"ANALYSIS_ERROR: {str(e)[:30]}"
        
        debug_results.append("=" * 60)
        debug_results.append("FEATURE STATUS SUMMARY")
        debug_results.append("=" * 60)
        
        # Count feature statuses with comprehensive analysis
        success_count = sum(1 for status in feature_status.values() if "SUCCESS" in status)
        zero_count = sum(1 for status in feature_status.values() if "ZERO_VALUES" in status)
        error_count = sum(1 for status in feature_status.values() if "ERROR" in status)
        missing_count = sum(1 for status in feature_status.values() if "MISSING" in status or "NOT_CREATED" in status)
        nan_count = sum(1 for status in feature_status.values() if "ALL_NAN" in status)
        categorical_count = sum(1 for status in feature_status.values() if "Categorical" in status)
        
        debug_results.append(f"SUCCESS: {success_count} features")
        debug_results.append(f"ZERO VALUES: {zero_count} features")
        debug_results.append(f"ERRORS: {error_count} features")
        debug_results.append(f"MISSING/NOT_CREATED: {missing_count} features")
        debug_results.append(f"ALL_NAN: {nan_count} features")
        debug_results.append(f"CATEGORICAL/DATETIME: {categorical_count} features")
        debug_results.append(f"TOTAL FEATURES ANALYZED: {len(feature_status)}")
        
        # List features with issues (excluding expected zeros)
        debug_results.append("")
        debug_results.append("FEATURES WITH ISSUES:")
        debug_results.append("-" * 40)
        # Exclude draw features as they're expected to be zero
        expected_zeros = ["r_draws_corrected", "b_draws_corrected", "draws_diff_corrected"]
        fallback_features = [f for f, s in feature_status.items() 
                           if ("FALLBACK" in s or "ERROR" in s or "MISSING" in s or "ZERO_VALUES" in s) 
                           and f not in expected_zeros]
        
        if fallback_features:
            for i in range(0, len(fallback_features), 5):
                line_features = fallback_features[i:i+5]
                debug_results.append("  " + " | ".join(line_features))
        else:
            debug_results.append("  None")
        
        debug_results.append("")
        debug_results.append("ZERO-VALUE FEATURE ANALYSIS:")
        debug_results.append("-" * 40)
        zero_features = []
        near_zero_features = []
        ...
        for feature in all_model_features:
            if feature in df.columns:
                # Skip datetime columns for sum calculations
                if df[feature].dtype.name.startswith('datetime'):
                    continue
                ...
                try:
                    ...
                    if feature_sum == 0:
                        ...
                        debug_results.append(f"  ⚠️ {feature}: All values are zero (sum=0)")
                    elif abs(feature_mean) < 0.001 and feature_std < 0.001:
                        near_zero_features.append(feature)
                        debug_results.append(f"  🔍 {feature}: Near-zero values (mean={feature_mean:.6f}, std={feature_std:.6f})")
                except Exception as e:
                    debug_results.append(f"  ⚠️ {feature}: Cannot analyze (dtype: {df[feature].dtype})")
        
        debug_results.append("")
        debug_results.append(f"📊 ZERO-VALUE SUMMARY:")
        debug_results.append(f"  Complete zeros: {len(zero_features)} features")
        debug_results.append(f"  Near zeros: {len(near_zero_features)} features")
        
        if zero_features:
            debug_results.append("")
            debug_results.append("🚨 COMPLETE ZERO FEATURES:")
            for i, feature in enumerate(zero_features, 1):
                debug_results.append(f"  {i:3d}. {feature}")
        
        if near_zero_features:
            debug_results.append("")
            debug_results.append("🔍 NEAR-ZERO FEATURES:")
            for i, feature in enumerate(near_zero_features, 1):
                debug_results.append(f"  {i:3d}. {feature}")
        
        # Check for features that should exist but don't
        debug_results.append("")
        debug_results.append("🔍 MISSING FEATURE CHECK:")
        debug_results.append("-" * 30)
        # All expected features to check
        expected_features = [
            "stance_matchup_advantage", "stance_versatility_advantage", "stance_advantage",
            "stamina_factor_diff", "pressure_performance_diff", "comeback_ability_diff",
            "clutch_factor_diff", "recent_trend_diff", "momentum_shift_diff",
            "form_consistency_diff", "upset_history_diff", "power_strike_ratio_diff",
            "strike_efficiency_gap", "damage_ratio_diff", "takedown_efficiency_diff",
            "ground_control_ratio_diff", "grappling_balance_diff", "defensive_ratio_diff",
            "takedown_defense_strength_diff", "overall_defense_score_diff", "experience_factor_diff",
            "win_rate_consistency_diff", "finish_rate_diff",
            "orthodox_vs_switch", "southpaw_vs_switch", "orthodox_vs_southpaw",
            "reach_utilization_diff", "size_advantage_factor", "title_fight_experience_diff",
            "main_event_pressure_diff", "head_hunting_ratio_diff",
            "body_work_frequency_diff", "leg_attack_frequency_diff", "recent_activity_level_diff",
            "form_consistency_alt_diff", "momentum_factor_diff"
        ]
        
        missing_expected = []
        for feature in expected_features:
            if feature not in df.columns:
                missing_expected.append(feature)
                debug_results.append(f"  ❌ {feature}: Missing from DataFrame")
        
        if missing_expected:
            debug_results.append(f"")
            debug_results.append(f"⚠️ FOUND {len(missing_expected)} MISSING EXPECTED FEATURES")
        else:
            debug_results.append("✅ All expected features are present")
        
        # Analyze which missing features are truly impossible
        debug_results.append("")
        debug_results.append("🔍 IMPOSSIBILITY ANALYSIS:")
        debug_results.append("-" * 35)
        
        impossible_features = []
        possible_but_missing = []
        
        for feature in missing_expected:
            # Analyze if feature is impossible based on available data
            if "round_specialist" in feature:
                # Check if we have round-by-round data
                if "finish_round" in df.columns:
                    possible_but_missing.append(feature)
                    debug_results.append(f"  ⚠️ {feature}: Possible - has finish_round data")
                else:
                    impossible_features.append(feature)
                    debug_results.append(f"  ❌ {feature}: Impossible - no round performance data")
            elif "head_hunting" in feature or "body_work" in feature or "leg_attack" in feature:
                # Check if we have strike target data
                if any(col in df.columns for col in ["r_head", "b_head", "r_body", "b_body", "r_leg", "b_leg"]):
                    possible_but_missing.append(feature)
                    debug_results.append(f"  ⚠️ {feature}: Possible - has strike target data")
                else:
                    impossible_features.append(feature)
                    debug_results.append(f"  ❌ {feature}: Impossible - no strike target data")
            elif "reach_utilization" in feature or "size_advantage" in feature:
                # Check if we have reach/height data
                if any(col in df.columns for col in ["r_reach", "b_reach", "r_height", "b_height"]):
                    possible_but_missing.append(feature)
                    debug_results.append(f"  ⚠️ {feature}: Possible - has reach/height data")
                else:
                    impossible_features.append(feature)
                    debug_results.append(f"  ❌ {feature}: Impossible - no reach/height data")
            else:
                # Default to possible for other features
                possible_but_missing.append(feature)
                debug_results.append(f"  ⚠️ {feature}: Possible - should be creatable")
        
        debug_results.append(f"")
        debug_results.append(f"📊 ANALYSIS RESULTS:")
        debug_results.append(f"  ❌ Truly impossible: {len(impossible_features)} features")
        debug_results.append(f"  ⚠️ Possible but missing: {len(possible_but_missing)} features")
        
        # Add critical dependencies check to final report
        debug_results.append("")
        debug_results.append("🔍 CRITICAL DEPENDENCIES CHECK:")
        debug_results.append("-" * 35)
        if missing_critical:
            debug_results.append(f"❌ MISSING CRITICAL COLUMNS: {len(missing_critical)}")
            for col in missing_critical:
                debug_results.append(f"  ❌ {col}")
        else:
            debug_results.append("✅ All critical dependencies present")
        ...
        debug_results.append("")
        debug_results.append("💾 Debug results saved to: Debugging_results.txt")
        debug_results.append("=" * 80)
        ...
        # REACH UTILIZATION DIFFERENTIAL
        df["r_reach_utilization"] = df["r_pro_SLpM_corrected"] * df["r_reach"] / 100
        df["b_reach_utilization"] = df["b_pro_SLpM_corrected"] * df["b_reach"] / 100
        df["reach_utilization_diff"] = df["r_reach_utilization"] - df["b_reach_utilization"]
        
        # SIZE ADVANTAGE FACTOR
        df["size_advantage_factor"] = (df["r_height"] - df["b_height"]) * (df["r_reach"] - df["b_reach"]) / 1000
        
        # TITLE FIGHT EXPERIENCE DIFFERENTIAL
        df["r_title_fight_experience"] = np.where(df["is_title_bout"] == 1, df["r_total_fights"] * 0.1, 0)
        df["b_title_fight_experience"] = np.where(df["is_title_bout"] == 1, df["b_total_fights"] * 0.1, 0)
        df["title_fight_experience_diff"] = df["r_title_fight_experience"] - df["b_title_fight_experience"]
        
        # ROUND SPECIALIST DIFFERENTIAL
        df["r_round_specialist"] = np.where(df["total_rounds"] == 5, df["r_avg_fight_time"] / 15, df["r_avg_fight_time"] / 9)
        df["b_round_specialist"] = np.where(df["total_rounds"] == 5, df["b_avg_fight_time"] / 15, df["b_avg_fight_time"] / 9)
        
        # MAIN EVENT PRESSURE DIFFERENTIAL
        df["r_main_event_pressure"] = df["r_recent_form_corrected"] * df["is_title_bout"] * 1.2
        df["b_main_event_pressure"] = df["b_recent_form_corrected"] * df["is_title_bout"] * 1.2
        df["main_event_pressure_diff"] = df["r_main_event_pressure"] - df["b_main_event_pressure"]
        
        # HEAD HUNTING RATIO DIFFERENTIAL
        df["r_head_hunting_ratio"] = df["r_head_pct_corrected"] * df["r_pro_SLpM_corrected"]
        df["b_head_hunting_ratio"] = df["b_head_pct_corrected"] * df["b_pro_SLpM_corrected"]
        df["head_hunting_ratio_diff"] = df["r_head_hunting_ratio"] - df["b_head_hunting_ratio"]
        
        # BODY WORK FREQUENCY DIFFERENTIAL
        df["r_body_work_frequency"] = df["r_body_pct_corrected"] * df["r_pro_SLpM_corrected"]
        df["b_body_work_frequency"] = df["b_body_pct_corrected"] * df["b_pro_SLpM_corrected"]
        df["body_work_frequency_diff"] = df["r_body_work_frequency"] - df["b_body_work_frequency"]
        
        # LEG ATTACK FREQUENCY DIFFERENTIAL
        df["r_leg_attack_frequency"] = df["r_leg_pct_corrected"] * df["r_pro_SLpM_corrected"]
        df["b_leg_attack_frequency"] = df["b_leg_pct_corrected"] * df["b_pro_SLpM_corrected"]
        df["leg_attack_frequency_diff"] = df["r_leg_attack_frequency"] - df["b_leg_attack_frequency"]
        
        # RECENT ACTIVITY LEVEL DIFFERENTIAL
        df["r_recent_activity_level"] = 1.0 / (df["r_days_since_last_fight_corrected"] + 1)
        df["b_recent_activity_level"] = 1.0 / (df["b_days_since_last_fight_corrected"] + 1)
        df["recent_activity_level_diff"] = df["r_recent_activity_level"] - df["b_recent_activity_level"]
        
        # FORM CONSISTENCY ALTERNATIVE DIFFERENTIAL
        df["r_form_consistency_alt"] = 1.0 - np.abs(df["r_recent_form_corrected"] - 0.5)
        df["b_form_consistency_alt"] = 1.0 - np.abs(df["b_recent_form_corrected"] - 0.5)
        df["form_consistency_alt_diff"] = df["r_form_consistency_alt"] - df["b_form_consistency_alt"]
        
        # MOMENTUM FACTOR DIFFERENTIAL
        df["r_momentum_factor"] = df["r_win_streak_corrected"] * df["r_recent_form_corrected"]
        df["b_momentum_factor"] = df["b_win_streak_corrected"] * df["b_recent_form_corrected"]
        df["momentum_factor_diff"] = df["r_momentum_factor"] - df["b_momentum_factor"]
        ...
        # Always create stance_matchup_advantage to avoid KeyError
        ...
        # COMPREHENSIVE FEATURE DEBUGGING SYSTEM
        debug_results = []
        debug_results.append("=" * 80)
        debug_results.append("🔍 COMPREHENSIVE FEATURE DEBUGGING REPORT")
        debug_results.append("=" * 80)
        debug_results.append(f"DataFrame shape: {df.shape}")
        debug_results.append(f"Total features to be created: {len(feature_columns)}")
        debug_results.append("")
        
        # Track feature creation success/failure
        feature_status = {}
        
        # Create stance features with safe fallback - ALWAYS create these features
        
        try:
            if "r_stance" in df.columns and "b_stance" in df.columns:
                debug_results.append("✅ Stance columns found - creating real stance features")
                
                # Debug stance data
                debug_results.append(f"r_stance unique values: {df['r_stance'].unique()}")
                debug_results.append(f"b_stance unique values: {df['b_stance'].unique()}")
                
                # Create stance matchup advantage with proper error handling
                def safe_stance_advantage(r_stance, b_stance):
                    try:
                        result = calculate_stance_advantage(r_stance, b_stance)
                        return result
                    except Exception as e:
                        debug_results.append(f"Stance error: {r_stance} vs {b_stance} - {e}")
                        return 0.0
                
                df["stance_matchup_advantage"] = df.apply(
                    lambda row: safe_stance_advantage(row["r_stance"], row["b_stance"]),
                    axis=1,
                )
                
                # Debug stance results
                debug_results.append(f"stance_matchup_advantage sum: {df['stance_matchup_advantage'].sum()}")
                debug_results.append(f"stance_matchup_advantage unique: {df['stance_matchup_advantage'].unique()[:10]}")
                ...
                stance_matchup_sum = df["stance_matchup_advantage"].sum()
                stance_versatility_sum = df["stance_versatility_advantage"].sum()
                debug_results.append(f"stance_matchup_advantage sum: {stance_matchup_sum}")
                debug_results.append(f"stance_versatility_advantage sum: {stance_versatility_sum}")
                
                if stance_matchup_sum != 0:
                    feature_status["stance_matchup_advantage"] = "SUCCESS"
                else:
                    feature_status["stance_matchup_advantage"] = "ZERO_VALUES"
                    debug_results.append("⚠️ WARNING: stance_matchup_advantage is all zeros")
                
                if stance_versatility_sum != 0:
                    feature_status["stance_versatility_advantage"] = "SUCCESS"
                else:
                    feature_status["stance_versatility_advantage"] = "ZERO_VALUES"
                    debug_results.append("⚠️ WARNING: stance_versatility_advantage is all zeros")
                    
            else:
                # Safe fallback - create with default values
                debug_results.append("⚠️ WARNING: Stance columns not found - using default values (0)")
                df["stance_matchup_advantage"] = 0
                df["stance_versatility_advantage"] = 0
                feature_status["stance_matchup_advantage"] = "FALLBACK_TO_ZERO"
                feature_status["stance_versatility_advantage"] = "FALLBACK_TO_ZERO"
        except Exception as e:
            # Ultimate fallback - ensure features are always created
            debug_results.append(f"❌ ERROR: Stance features creation failed: {e}")
            df["stance_matchup_advantage"] = 0
            df["stance_versatility_advantage"] = 0
            feature_status["stance_matchup_advantage"] = "ERROR_FALLBACK_TO_ZERO"
            feature_status["stance_versatility_advantage"] = "ERROR_FALLBACK_TO_ZERO"
        ...
        try:
            ...
            feature_status["stance_versatility_impact"] = "SUCCESS"
        except Exception as e:
            debug_results.append(f"❌ ERROR: stance_versatility_impact creation failed: {e}")
            df["stance_versatility_impact"] = 0
            feature_status["stance_versatility_impact"] = "ERROR_FALLBACK_TO_ZERO"
        
        # Check critical dependencies
        critical_columns = [
            "r_pro_SLpM_corrected", "b_pro_SLpM_corrected",
            ...
            "r_recent_form_corrected", "b_recent_form_corrected",
            "r_win_streak_corrected", "b_win_streak_corrected",
            "r_loss_streak_corrected", "b_loss_streak_corrected"
        ]
        
        # Track critical dependencies for final report
        missing_critical = []
        ...
        feature_columns.extend(
            [
                ...
                "upset_history_diff",
                # 26 NEW HIGH-IMPACT DIFFERENTIAL FEATURES
                "power_strike_ratio_diff",
                "strike_efficiency_gap",
                "damage_ratio_diff",
                "takedown_efficiency_diff",
                "ground_control_ratio_diff",
                "grappling_balance_diff",
                "defensive_ratio_diff",
                "takedown_defense_strength_diff",
                "overall_defense_score_diff",
                "experience_factor_diff",
                "win_rate_consistency_diff",
                # REMAINING 15 HIGH-IMPACT FEATURES
                "finish_rate_diff",
                "stance_advantage",
                "orthodox_vs_switch",
                ...
                "reach_utilization_diff",
                "size_advantage_factor",
                "title_fight_experience_diff",
                "main_event_pressure_diff",
                "head_hunting_ratio_diff",
                "body_work_frequency_diff",
                "leg_attack_frequency_diff",
                "recent_activity_level_diff",
                "form_consistency_alt_diff",
                "momentum_factor_diff",
            ]
        )
    ...
    def prepare_upcoming_fight(self, fight, feature_columns):
        ...
        # Championship experience based on title fights and 5-round experience
        r_championship_exp = (
            1 if r_stats.get("r_championship_experience", 0) > 0 else 0
        )
        b_championship_exp = (
            1 if b_stats.get("b_championship_experience", 0) > 0 else 0
        )
        ...
        # STAMINA FACTOR - Based on fight duration patterns
        r_stamina_factor = 1.2 if r_avg_fight_time > 12 else (1.0 if r_avg_fight_time > 8 else 0.8)
        b_stamina_factor = 1.2 if b_avg_fight_time > 12 else (1.0 if b_avg_fight_time > 8 else 0.8)
        fight_features["stamina_factor_diff"] = r_stamina_factor - b_stamina_factor
        
        # PRESSURE PERFORMANCE - Performance in high-stakes fights
        r_pressure_performance = r_stats["recent_form"] * 1.2 if fight_features["is_title_bout"] == 1 else r_stats["recent_form"]
        b_pressure_performance = b_stats["recent_form"] * 1.2 if fight_features["is_title_bout"] == 1 else b_stats["recent_form"]
        fight_features["pressure_performance_diff"] = r_pressure_performance - b_pressure_performance
        
        # COMEBACK ABILITY - Ability to recover from adversity
        r_comeback_ability = r_stats["recent_form"] * 0.8 if r_stats["loss_streak"] > 0 else r_stats["recent_form"] * 1.1
        b_comeback_ability = b_stats["recent_form"] * 0.8 if b_stats["loss_streak"] > 0 else b_stats["recent_form"] * 1.1
        fight_features["comeback_ability_diff"] = r_comeback_ability - b_comeback_ability
        
        # CLUTCH FACTOR - Performance in close fights
        r_clutch_factor = r_stats["win_loss_ratio"] * r_stats["recent_form"]
        b_clutch_factor = b_stats["win_loss_ratio"] * b_stats["recent_form"]
        fight_features["clutch_factor_diff"] = r_clutch_factor - b_clutch_factor
        
        # RECENT TREND - Performance trajectory over last 3-5 fights
        r_recent_trend = r_stats["recent_form"] * r_stats["win_streak"]
        b_recent_trend = b_stats["recent_form"] * b_stats["win_streak"]
        fight_features["recent_trend_diff"] = r_recent_trend - b_recent_trend
        
        # MOMENTUM SHIFT - Change in performance momentum
        r_momentum_shift = r_stats["win_streak"] - r_stats["loss_streak"]
        b_momentum_shift = b_stats["win_streak"] - b_stats["loss_streak"]
        fight_features["momentum_shift_diff"] = r_momentum_shift - b_momentum_shift
        
        # FORM CONSISTENCY - How consistent recent performance is
        r_form_consistency = 1.0 - abs(r_stats["recent_form"] - 0.5) * 2
        b_form_consistency = 1.0 - abs(b_stats["recent_form"] - 0.5) * 2
        fight_features["form_consistency_diff"] = r_form_consistency - b_form_consistency
        
        # UPSET HISTORY - History of causing/being upset
        r_upset_history = 1.2 if r_stats["recent_form"] > 0.7 else (0.8 if r_stats["recent_form"] < 0.3 else 1.0)
        b_upset_history = 1.2 if b_stats["recent_form"] > 0.7 else (0.8 if b_stats["recent_form"] < 0.3 else 1.0)
        fight_features["upset_history_diff"] = r_upset_history - b_upset_history
        ...
        # POWER VS VOLUME ANALYSIS
        r_power_strike_ratio = r_stats["pro_kd_pM"] / (r_stats["pro_SLpM"] + 0.001)
        b_power_strike_ratio = b_stats["pro_kd_pM"] / (b_stats["pro_SLpM"] + 0.001)
        fight_features["power_strike_ratio_diff"] = r_power_strike_ratio - b_power_strike_ratio
        
        # STRIKE EFFICIENCY GAP
        r_strike_efficiency = r_stats["pro_sig_str_acc"] - r_stats["pro_SApM"]
        b_strike_efficiency = b_stats["pro_sig_str_acc"] - b_stats["pro_SApM"]
        fight_features["strike_efficiency_gap"] = r_strike_efficiency - b_strike_efficiency
        
        # DAMAGE RATIO
        r_damage_ratio = r_stats["pro_SLpM"] / (r_stats["pro_SApM"] + 0.001)
        b_damage_ratio = b_stats["pro_SLpM"] / (b_stats["pro_SApM"] + 0.001)
        fight_features["damage_ratio_diff"] = r_damage_ratio - b_damage_ratio
        
        # TAKEDOWN EFFICIENCY
        r_takedown_efficiency = r_stats["pro_td_acc"] * r_stats["pro_td_avg"]
        b_takedown_efficiency = b_stats["pro_td_acc"] * b_stats["pro_td_avg"]
        fight_features["takedown_efficiency_diff"] = r_takedown_efficiency - b_takedown_efficiency
        
        # GROUND CONTROL RATIO
        r_ground_control_ratio = r_stats["pro_td_avg"] / (r_stats["pro_sub_avg"] + 0.001)
        b_ground_control_ratio = b_stats["pro_td_avg"] / (b_stats["pro_sub_avg"] + 0.001)
        fight_features["ground_control_ratio_diff"] = r_ground_control_ratio - b_ground_control_ratio
        
        # GRAPPLING BALANCE
        r_grappling_balance = (r_stats["pro_td_avg"] + r_stats["pro_sub_avg"]) / 2
        b_grappling_balance = (b_stats["pro_td_avg"] + b_stats["pro_sub_avg"]) / 2
        fight_features["grappling_balance_diff"] = r_grappling_balance - b_grappling_balance
        
        # DEFENSIVE RATIO
        r_defensive_ratio = r_stats["pro_str_def"] / (r_stats["pro_SApM"] + 0.001)
        b_defensive_ratio = b_stats["pro_str_def"] / (b_stats["pro_SApM"] + 0.001)
        fight_features["defensive_ratio_diff"] = r_defensive_ratio - b_defensive_ratio
        
        # TAKEDOWN DEFENSE STRENGTH
        r_takedown_defense_strength = r_stats["pro_td_def"] * (1 - r_stats["pro_SApM"])
        b_takedown_defense_strength = b_stats["pro_td_def"] * (1 - b_stats["pro_SApM"])
        fight_features["takedown_defense_strength_diff"] = r_takedown_defense_strength - b_takedown_defense_strength
        
        # OVERALL DEFENSE SCORE
        r_overall_defense_score = (r_stats["pro_str_def"] + r_stats["pro_td_def"]) / 2
        b_overall_defense_score = (b_stats["pro_str_def"] + b_stats["pro_td_def"]) / 2
        fight_features["overall_defense_score_diff"] = r_overall_defense_score - b_overall_defense_score
        
        # EXPERIENCE FACTOR
        r_experience_factor = r_total_fights / (r_stats["age_at_event"] + 0.001)
        b_experience_factor = b_total_fights / (b_stats["age_at_event"] + 0.001)
        fight_features["experience_factor_diff"] = r_experience_factor - b_experience_factor
        
        # WIN RATE CONSISTENCY
        r_win_rate_consistency = r_stats["wins"] / (r_stats["wins"] + r_stats["losses"] + 0.001)
        b_win_rate_consistency = b_stats["wins"] / (b_stats["wins"] + b_stats["losses"] + 0.001)
        fight_features["win_rate_consistency_diff"] = r_win_rate_consistency - b_win_rate_consistency
        ...
        # FINISH RATE DIFFERENTIAL
        r_finish_rate = (r_stats["ko_rate"] + r_stats["sub_rate"]) / 2
        b_finish_rate = (b_stats["ko_rate"] + b_stats["sub_rate"]) / 2
        fight_features["finish_rate_diff"] = r_finish_rate - b_finish_rate
        
        # STANCE MATCHUP ADVANTAGE
        def calculate_stance_advantage(r_stance, b_stance):
            # Based on real-world fight data analysis
            if r_stance == "Orthodox" and b_stance == "Southpaw":
                return -0.06  # Southpaw has 6% advantage (52% vs 46%)
        ...
        fight_features["stance_matchup_advantage"] = calculate_stance_advantage(r_stats["stance"], b_stats["stance"])
        
        # STANCE VERSATILITY ADVANTAGE
        if r_stats["stance"] == "Switch":
            fight_features["stance_versatility_advantage"] = 0.14
        elif b_stats["stance"] == "Switch":
            fight_features["stance_versatility_advantage"] = -0.14
        else:
            fight_features["stance_versatility_advantage"] = 0
        
        # Add missing high-impact features for prediction
        # ORTHODOX VS SWITCH
        if r_stats["stance"] == "Orthodox" and b_stats["stance"] == "Switch":
            fight_features["orthodox_vs_switch"] = 0.15
        elif r_stats["stance"] == "Switch" and b_stats["stance"] == "Orthodox":
            fight_features["orthodox_vs_switch"] = -0.15
        else:
            fight_features["orthodox_vs_switch"] = 0
        
        # SOUTHPAW VS SWITCH
        if r_stats["stance"] == "Southpaw" and b_stats["stance"] == "Switch":
            fight_features["southpaw_vs_switch"] = 0.13
        elif r_stats["stance"] == "Switch" and b_stats["stance"] == "Southpaw":
            fight_features["southpaw_vs_switch"] = -0.13
        else:
            fight_features["southpaw_vs_switch"] = 0
        
        # ORTHODOX VS SOUTHPAW
        if r_stats["stance"] == "Orthodox" and b_stats["stance"] == "Southpaw":
            fight_features["orthodox_vs_southpaw"] = -0.06
        elif r_stats["stance"] == "Southpaw" and b_stats["stance"] == "Orthodox":
            fight_features["orthodox_vs_southpaw"] = 0.06
        else:
            fight_features["orthodox_vs_southpaw"] = 0
        ...
        reach_diff = r_stats["reach"] - b_stats["reach"]
        avg_reach = (r_stats["reach"] + b_stats["reach"]) / 2
        fight_features["reach_utilization_diff"] = safe_divide(reach_diff, avg_reach, 0)
        
        # SIZE ADVANTAGE FACTOR
        fight_features["size_advantage_factor"] = (
            (r_stats["height"] - b_stats["height"]) * 0.3 + 
            (r_stats["reach"] - b_stats["reach"]) * 0.7
        ) / 100
        
        # TITLE FIGHT EXPERIENCE DIFFERENTIAL
        fight_features["title_fight_experience_diff"] = (
            r_stats.get("r_championship_experience", 0) - b_stats.get("b_championship_experience", 0)
        )
        
        
        # HEAD HUNTING RATIO DIFFERENTIAL
        r_head_ratio = safe_divide(r_stats["head"], r_stats["sig_str"], 0)
        b_head_ratio = safe_divide(b_stats["head"], b_stats["sig_str"], 0)
        fight_features["head_hunting_ratio_diff"] = r_head_ratio - b_head_ratio
        
        # BODY WORK FREQUENCY DIFFERENTIAL
        r_body_ratio = safe_divide(r_stats["body"], r_stats["sig_str"], 0)
        b_body_ratio = safe_divide(b_stats["body"], b_stats["sig_str"], 0)
        fight_features["body_work_frequency_diff"] = r_body_ratio - b_body_ratio
        
        # LEG ATTACK FREQUENCY DIFFERENTIAL
        r_leg_ratio = safe_divide(r_stats["leg"], r_stats["sig_str"], 0)
        b_leg_ratio = safe_divide(b_stats["leg"], b_stats["sig_str"], 0)
        fight_features["leg_attack_frequency_diff"] = r_leg_ratio - b_leg_ratio
        
        # RECENT ACTIVITY LEVEL DIFFERENTIAL
        fight_features["recent_activity_level_diff"] = (
            r_stats["days_since_last_fight"] - b_stats["days_since_last_fight"]
        ) / 30
        
        # FORM CONSISTENCY ALTERNATIVE DIFFERENTIAL
        fight_features["form_consistency_alt_diff"] = (
            r_stats["recent_form"] - b_stats["recent_form"]
        ) * 0.5
        
        # MOMENTUM FACTOR DIFFERENTIAL
        fight_features["momentum_factor_diff"] = (
            r_stats["momentum_score"] - b_stats["momentum_score"]
        ) * 0.3
        
        # STANCE ADVANTAGE (use existing stance_matchup_advantage)
        fight_features["stance_advantage"] = fight_features["stance_matchup_advantage"]
        
        # STRIKER VS GRAPPLER
        striker_advantage = r_stats["pro_SLpM"] - b_stats["pro_SLpM"]
        grappler_advantage = r_stats["pro_td_avg"] - b_stats["pro_td_avg"]
        fight_features["striker_vs_grappler"] = np.where(
            (striker_advantage > 0.3) & (grappler_advantage < -0.3),
            1,
            np.where(
                (striker_advantage < -0.3) & (grappler_advantage > 0.3),
                -1,
        ...
        fight_features["style_clash_severity"] = abs(
            fight_features["striker_vs_grappler"] * 0.5
            + (r_stats["pro_SLpM"] - b_stats["pro_SLpM"]) * 0.3
            + (r_stats["pro_td_avg"] - b_stats["pro_td_avg"]) * 0.2
        )
        
        # ORTHODOX VS SWITCH
        fight_features["orthodox_vs_switch"] = -0.15 if (r_stats["stance"] == "Orthodox" and b_stats["stance"] == "Switch") else (0.15 if (r_stats["stance"] == "Switch" and b_stats["stance"] == "Orthodox") else 0)
        
        # SOUTHPAW VS SWITCH
        fight_features["southpaw_vs_switch"] = -0.13 if (r_stats["stance"] == "Southpaw" and b_stats["stance"] == "Switch") else (0.13 if (r_stats["stance"] == "Switch" and b_stats["stance"] == "Southpaw") else 0)
        
        # ORTHODOX VS SOUTHPAW
        fight_features["orthodox_vs_southpaw"] = -0.06 if (r_stats["stance"] == "Orthodox" and b_stats["stance"] == "Southpaw") else (0.06 if (r_stats["stance"] == "Southpaw" and b_stats["stance"] == "Orthodox") else 0)
        
        # REACH UTILIZATION DIFFERENTIAL
        r_reach_utilization = r_stats["pro_SLpM"] * r_stats["reach"] / 100
        b_reach_utilization = b_stats["pro_SLpM"] * b_stats["reach"] / 100
        fight_features["reach_utilization_diff"] = r_reach_utilization - b_reach_utilization
        
        # SIZE ADVANTAGE FACTOR
        fight_features["size_advantage_factor"] = (r_stats["height"] - b_stats["height"]) * (r_stats["reach"] - b_stats["reach"]) / 1000
        
        # TITLE FIGHT EXPERIENCE DIFFERENTIAL
        r_title_fight_experience = r_total_fights * 0.1 if fight_features["is_title_bout"] == 1 else 0
        b_title_fight_experience = b_total_fights * 0.1 if fight_features["is_title_bout"] == 1 else 0
        fight_features["title_fight_experience_diff"] = r_title_fight_experience - b_title_fight_experience
        
        
        # MAIN EVENT PRESSURE DIFFERENTIAL
        r_main_event_pressure = r_stats["recent_form"] * fight_features["is_title_bout"] * 1.2
        b_main_event_pressure = b_stats["recent_form"] * fight_features["is_title_bout"] * 1.2
        fight_features["main_event_pressure_diff"] = r_main_event_pressure - b_main_event_pressure
        
        # HEAD HUNTING RATIO DIFFERENTIAL
        r_head_hunting_ratio = r_stats["head_pct"] * r_stats["pro_SLpM"]
        b_head_hunting_ratio = b_stats["head_pct"] * b_stats["pro_SLpM"]
        fight_features["head_hunting_ratio_diff"] = r_head_hunting_ratio - b_head_hunting_ratio
        
        # BODY WORK FREQUENCY DIFFERENTIAL
        r_body_work_frequency = r_stats["body_pct"] * r_stats["pro_SLpM"]
        b_body_work_frequency = b_stats["body_pct"] * b_stats["pro_SLpM"]
        fight_features["body_work_frequency_diff"] = r_body_work_frequency - b_body_work_frequency
        
        # LEG ATTACK FREQUENCY DIFFERENTIAL
        r_leg_attack_frequency = r_stats["leg_pct"] * r_stats["pro_SLpM"]
        b_leg_attack_frequency = b_stats["leg_pct"] * b_stats["pro_SLpM"]
        fight_features["leg_attack_frequency_diff"] = r_leg_attack_frequency - b_leg_attack_frequency
        
        # RECENT ACTIVITY LEVEL DIFFERENTIAL
        r_recent_activity_level = 1.0 / (r_stats["days_since_last_fight"] + 1)
        b_recent_activity_level = 1.0 / (b_stats["days_since_last_fight"] + 1)
        fight_features["recent_activity_level_diff"] = r_recent_activity_level - b_recent_activity_level
        
        # FORM CONSISTENCY ALTERNATIVE DIFFERENTIAL
        r_form_consistency_alt = 1.0 - abs(r_stats["recent_form"] - 0.5)
        b_form_consistency_alt = 1.0 - abs(b_stats["recent_form"] - 0.5)
        fight_features["form_consistency_alt_diff"] = r_form_consistency_alt - b_form_consistency_alt
        
        # MOMENTUM FACTOR DIFFERENTIAL
        r_momentum_factor = r_stats["win_streak"] * r_stats["recent_form"]
        b_momentum_factor = b_stats["win_streak"] * b_stats["recent_form"]
        fight_features["momentum_factor_diff"] = r_momentum_factor - b_momentum_factor
